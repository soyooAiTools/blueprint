...
- N=10 cold-export corpus is now a file-level gate; default programmer-delivery export still remains `gmp-v14` until a separate explicit cutover decision and Editor hydration certification path are complete.
...
- v0: N=5 cold exports passed as internal profile only.
- v1: N=10 cold exports passed as file-level gate before default programmer-delivery prompt cutover.
- v2: N=20 broader corpus for stronger confidence.

Current executable coverage:

- `test/delivery-spec-source-parity.test.cjs` covers SourceIR -> UnityDeliverySpec
  semantic parity.
- `test/unitycomponent-v1-cold-export-corpus.test.cjs` covers N=10
  SourceIR-artifacts -> UnityComponent v1 Unity export -> v1 hardgate.
...
- `scripts/validation-router.cjs` routes `scripts/export-unity-project.sh`
  changes through both legacy programmer-delivery gates and the
  unitycomponent-v1 profile/projector/emitter/hardgate/cold-corpus gates.
