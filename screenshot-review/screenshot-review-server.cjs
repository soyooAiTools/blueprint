...
const server = http.createServer(async (req, res) => {
  ...
  if (match && req.method === 'POST') {
    ...
    try {
      ...
      // Pixel-level blank screen check (hard reject, no AI override)
      if (ssResult.blankDetection && ssResult.blankDetection.blank) {
        console.log(`[${new Date().toISOString()}] BLANK SCREEN detected: ${JSON.stringify(ssResult.blankDetection)}`);
        res.writeHead(200);
        res.end(JSON.stringify({
          ok: false,
          decision: 'REJECT',
          reason: '白屏/黑屏/纯色检测: ' + ssResult.blankDetection.reason + ' (unique colors: ' + ssResult.blankDetection.uniqueColors + ')',
          blank: true,
          blankDetection: ssResult.blankDetection,
