...
function getFixHint(type) {
  ...
  var hints = {
    ...
    'batch-completion': 'Multiple phases completed in a single poll interval. Each phase must hold a distinct visual state for the 12s AutoPlay dwell gate. Check that CheckEventRules uses PhaseDwellReady(Nf) and that AutoPlay dwell is backed by phaseRealTimer/Time.realtimeSinceStartup, not accelerated phaseTimer.' + LUNA_REMINDERS,
