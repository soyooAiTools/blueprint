...
function getFixHint(type) {
  var LUNA_REMINDERS = '\nLuna SDK reminders: NEVER use SetActive() (causes objects to vanish). NEVER use CreatePrimitive(). ' +
    'Luna/WebGL staging pool objects are named __Pool_{Shape}_{Color}_{NN} and start at (0,-999,0); move pre-bound objects to visible positions with transform.position = new Vector3(x, y, z). ' +
    'Programmer delivery references are Inspector-hydrated refs such as GMP_SceneEntityRefs/serialized refs, not runtime scene scans. ' +
  ...
  var hints = {
    ...
    'visual-freeze': 'Game screen is visually STATIC despite phases completing. AutoPlay transitions must produce VISIBLE changes: (1) move pre-bound entities via transform.Translate or transform.position, (2) show/hide by moving bound objects between visible positions and the offscreen pool position, (3) use pre-baked pool colors or hydrated art refs instead of runtime material-color fixes, (4) update UI text/camera/phase evidence. Phases that only increment counters with no visual side effects will fail this check.' + LUNA_REMINDERS,
