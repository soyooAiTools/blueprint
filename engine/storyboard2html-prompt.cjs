...
var SYSTEM_PROMPT_HEADER = [
  ...
  '- HTML runtime 可以继续用 JS/Three.js,但视觉/UI/模型/phase driver 必须由 SourceIR preview renderer 从 `window.__BP_SOURCE_IR__` 派生;不要靠函数体 side effects 让 demo2spec 猜语义。',
  '',
...
var COMPACT_SYSTEM_PROMPT_HEADER = [
  ...
  '- HTML runtime 可继续用 JS/Three.js,但视觉/UI/模型/phase driver 必须由 SourceIR preview renderer 从 `window.__BP_SOURCE_IR__` 派生,不要靠函数体 side effects 让 demo2spec 猜语义。',
  '',
  '## L1 — HTML 静态入口',
