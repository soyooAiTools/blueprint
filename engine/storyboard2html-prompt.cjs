...
var SYSTEM_PROMPT_HEADER = [
  ...
  '  ```',
  '',
  '## L5 — 交互驱动硬约束(phase 推进必须由 *固定虚拟摇杆 + 角色 arrival-gate* 触发,*禁止键盘/直接 entity click / setTimeout 自走*)',
  '- *控制范式硬约束* = *固定虚拟摇杆*(屏幕固定位置,通常 left-bottom 角,圆形底盘 + 中心拖拽 thumb):',
  '  - 必须存在可见 UI 控件(`<div id="joystick">` 或 canvas 上层固定 layer),CSS `position:fixed` 锁住一角,不随场景滚动,不允许只是隐式键盘控制。',
  '  - 摇杆控件 *必须* 挂完整三段事件:`pointerdown` → `pointermove` → `pointerup`(等价 `touchstart`/`touchmove`/`touchend`),计算 thumb 偏移 → 写入 `__gameState.input.joystick = { dx, dy, active }`。',
  ...
  '- 不允许 non-final phase 完全没有 *固定虚拟摇杆 pointerdown+pointermove+pointerup 三段 listener + arrival 物理判定* 驱动 phase advance(每个 non-final phase 必须 = 摇杆 listener 推角色 + 角色到达目标圈 + 进入后触发 phase 逻辑;keyboard / 直接 entity click 都不算 canonical 驱动)。',
  '- 不允许整个 demo 缺失固定虚拟摇杆 UI 控件(必须 *屏幕固定位置*、圆盘 + 拖拽 thumb、`position:fixed` CSS、`pointerdown`+`pointermove`+`pointerup` 三段事件 listener,缺一即视为 prompt bug)。',
