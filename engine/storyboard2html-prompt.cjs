...
var SYSTEM_PROMPT_HEADER = [
  ...
  '',
  '## L0 — SourceSceneIR 语义源(新主事实,替代 JS 语义反推)',
  '- 顶层声明 `window.__BP_SOURCE_IR__ = { ... }`,schemaVersion 必须是 `"source-scene-ir.v1"`,kind 必须是 `"blueprint.sourceSceneIR"`。',
  '- 顶层声明 `window.__BP_SOURCE_IR_HASH__ = "<sha256>"`;该 hash 对 `__BP_SOURCE_IR__` 的 normalized semantic payload 计算,不包含 generatedAt/source/diagnostics/extraction。',
  '- 顶层声明 SourceIR preview renderer ownership:`window.__BP_SOURCE_IR_PREVIEW_RENDERER_VERSION__ = "source-ir-preview-renderer.v1"`、`window.__BP_SOURCE_IR_RENDERER_OWNS_VISUALS__ = true`、`window.__BP_SOURCE_IR_RENDERER_OWNS_PHASE_DRIVER__ = true`、`window.__BP_SOURCE_IR_VISUAL_SOURCE__ = "source-scene-ir"`。',
  '- `__BP_SOURCE_IR__` 必含 `project` / `scene` / `entities[]` / `resources[]` / `phases[]` / `hud` / `runtimeContract`。',
  '- `phases[].id` 必须严格连续 `phase1..phaseN`;`showEntities[]`、step target/from/to/entity、gate entity/resource 必须引用已声明 entity/resource。',
  '- `phases[].steps[].kind` / `step.kind` 只允许: `move_to` / `collect` / `deliver` / `build` / `upgrade` / `attack` / `spawn` / `despawn` / `show` / `hide` / `set_tip` / `set_resource` / `set_entity_state` / `wait` / `cta_finish`。',
  '- `phases[].gate.kind` 只允许: `near_entity` / `resource` / `entity_state` / `entity_count` / `timer` / `compound_all` / `compound_any` / `cta_arrival`。',
  '- non-final phase 禁止 CTA/click 完成语义:不要写 `cta_finish` step,不要写 `cta_arrival` gate,不要把 click 当 phase advance。',
  '- `runtimeContract.requiresJoystick=true` 时,HTML 必须有真实浮动摇杆 runtime evidence;`runtimeContract.forbidAutoplayProgress` 必须为 true。',
  '- 旧兼容变量必须从 SourceSceneIR 投影派生,不能另起一套事实:`PHASES = __BP_SOURCE_IR__.phases` 的兼容投影,`ENTITY_STYLE = entities[].visual` 投影,`ENTITY_POSITIONS = entities[].position` 投影,`SCENE_CONFIG = scene` 投影。',
  '- HTML runtime 可以继续用 JS/Three.js,但视觉/UI/模型/phase driver 必须由 SourceIR preview renderer 从 `window.__BP_SOURCE_IR__` 派生;不要靠函数体 side effects 让 SourceIR 猜语义。',
  '',
  '## L1 — HTML 静态入口(强约束,SourceIR 静态解析必须命中)',
  '- 顶层声明 `const PHASES = [...]`(或 `let`/`var`/`window.PHASES = [...]`)。',
  '- 每个 PHASES[i] 含字段:`id` (字符串 `phase{n}`)、`name`、`goalText`、`steps[]`;',
  '  推荐补:`guideText`、`durationSec`、`showEntities[]`、`trigger{type,...}`、`plannedModuleIds[]`。',
  '- `steps[]` 必须是该 phase 的完整用户体验目标序列,顺序与运行时 `setTarget(...)` / targetRing / targetHint 完全一致;不要只写 phase 最终出口。例:`[{target:"Gate",label:"升级大门",setEntity:"Gate",state:2},{target:"Enemy",label:"击败敌人",damage:true},{target:"Debris",label:"拾取残骸",gain:"Scrap",amount:1},{target:"Recycler",label:"送回收机"}]`。',
  ...
  '- SourceIR renderer 还必须挂 `window.__driveToSourcePhase = window.__driveToPhase` 的同形态源端 phase hook,供 preflight 明确区分源 HTML phase driver ownership。',
  '',
  ...
  '  这样 SourceIR 反解时能区分"LLM 忘了写 meta"(prompt bug)和"真的查不到 license"(合规 fallback)。',
  ...
  '  - 这是 SourceIR extractor 反向追踪 entity↔mesh 绑定的 *唯一* anchor 形态(没有它,extractor 抓到的 Mesh 全 unbound,Luna 端 fallback 默认 `__Pool_Cube_White_01`,所有 entity 都长成同款白方块)。',
  ...
  '  - `backgroundColor`: 数字 `0xRRGGBB`(scene 主色 / camera clear color)。**必填**,SourceIR extractor 直接给 Luna `cam.backgroundColor`。',
  ...
  '  - `camera`: `{ fov: <number 度>, position: [x,y,z], lookAt: [x,y,z], near?: <number>, far?: <number> }` 或 `null`(Option C source-faithful 用:源 PerspectiveCamera 的 FoV 与机位/朝向,缺省 null 时 Luna 沿用默认机位)。必须与源 HTML 实际 `camera.position.set(...)` / `camera.lookAt(...)` / `PerspectiveCamera(fov,...)` 字面一致。',
  '- 字段语义:Luna skeleton 解析 `sourceSceneContract`(由 SourceIR 从 `SCENE_CONFIG` 抽出)后,**按字段而不是按主题** hard-set 背景/光照/装饰 — 同一套契约支持太空 / 农场 / 餐厅 / 塔防 / 解谜 等任意主题,extractor 不需要主题判定,Luna runtime 不需要 if-theme-then-X 硬编码分支。',
  '- canonical 示例(深空太空主题):`const SCENE_CONFIG = { backgroundColor: 0x071026, fog: { color: 0x071026, near: 55, far: 145 }, ambientLight: { color: 0xffffff, intensity: 0.62 }, directionalLight: { color: 0xffffff, intensity: 1.25, position: [-14, 28, 18] }, rimLight: { color: 0x72ddff, intensity: 1.0, position: [10, 16, -16], distance: 80 }, ground: { kind: "cylinder", radius: 72, height: 0.25, color: 0x13233a }, decor: { stars: 100, orbitalRings: 4 }, camera: { fov: 60, position: [0, 18, 16], lookAt: [0, 0, 0] } };`',
  ...
  '- 必须在 scene 闭包顶层声明 `var targetRing, trailLine, laserLine;`,与 `entity_states/models/labels` 同级。SourceIR extractor 会按变量名识别并写入 `sourceSceneContract.guidance`,Luna overlay 必须重建这三件视觉。',
  ...
  '',
  '## L9 — Mesh-op 契约(Luna source-faithful build,Option C,受 OPTION_C_SOURCE_FAITHFUL_BUILD 灰度兜底)',
  '- *顶层* 再声明 `var meshOps = {}`(允许 `let` / `const` / `window.meshOps`,*不能* 闭包内):',
  '  - key 集合 = ENTITY_STYLE keys(完全一致;两边都得有)。',
  '  - value = 1..N 个 mesh-op 字面对象组成的纯数组;每个 op 描述一个原子 mesh,*无 lambda / closure / Math.random / Date.now / THREE 引用*。',
  '  - 必含字段:`kind`(`"box"|"sphere"|"cylinder"|"cone"|"plane"|"torus"|"icosahedron"`,全小写,Luna 端 switch dispatch);`position`(`[x,y,z]` 数值字面量,相对 entity 原点)。',
  '  - 可选字段:`rotation`(`[x,y,z]` 欧拉角 degree)、`scale`(`[x,y,z]`,默认 [1,1,1])、`size`(因 kind 而异:box→`[w,h,d]`,cylinder/cone→`[rTop,rBottom,h]`,sphere/icosahedron→`[r]`,plane→`[w,h]`,torus→`[radius,tube]`)、`color`(`0xRRGGBB`,默认继承 ENTITY_STYLE[name].color)、`emissive`(`0xRRGGBB`,默认 0x000000)、`emissiveIntensity`(0~1,默认 0)、`metalness`(0~1,默认 0)、`roughness`(0~1,默认 1)、`opacity`(0~1,默认 1)。',
  '- meshOps *不是替代* `buildEntity`,而是 *并行声明事实*:源 HTML 仍按现有 `buildEntity` 装配 three.js Mesh。推荐 helper:`function emitMeshOp(name, op){ meshOps[name] = meshOps[name] || []; meshOps[name].push(op); }`,在 `buildEntity` 内每个 `new THREE.Mesh(...)` 之后立刻 `emitMeshOp(name, {kind:"box", position:[0,1.15,0], size:[0.82,1.12,0.48], color:0x66ccff})`(数值全字面量)。',
  '- `meshOps` 内所有数值必须为字面量(Number / Array of Number / 0xRRGGBB),*不能* 引用 `ENTITY_STYLE[X].color` 等运行时表达式 —— extractor 跑一次 `page.evaluate(()=>window.meshOps)` 后 JSON 深拷贝,不解析任意 JS。',
  '- 禁止把 `meshOps` 写在 IIFE / scene 闭包 / function 内部;必须 *顶层* 声明,且在 `Object.keys(ENTITY_STYLE).forEach(buildEntity)` driver 跑过后也同步可读(静态字面填法或 runtime-populated 填法均可)。',
  '- 兜底:`meshOps[name]` 缺失或空数组时,Luna 端 fall back 到现有 `spec.entities[].visual.shape` pooled-primitive 路径(等价老行为;Option C flag off 时也走这条)。',
  '',
  '## 禁止反规则',
  ...
  '- 不允许缺失 `window.__BP_SOURCE_IR__`;新 storyboard2html 输出必须先声明 SourceSceneIR,再从它派生 PHASES / ENTITY_STYLE / ENTITY_POSITIONS / SCENE_CONFIG。',
  '- 不允许 `window.__BP_SOURCE_IR_HASH__` 与 normalized SourceSceneIR semantic payload 不一致;如果写了 hash,source-scene-ir preflight 会重算并 hard fail mismatch。',
  '- 不允许缺失 SourceIR preview renderer ownership 标识:`window.__BP_SOURCE_IR_PREVIEW_RENDERER_VERSION__` / `window.__BP_SOURCE_IR_RENDERER_OWNS_VISUALS__` / `window.__BP_SOURCE_IR_RENDERER_OWNS_PHASE_DRIVER__` / `window.__BP_SOURCE_IR_VISUAL_SOURCE__` / `window.__driveToSourcePhase`。',
  '- 不允许 `__BP_SOURCE_IR__.phases[].steps[].kind` / `gate.kind` 超出白名单;不要新增自由 DSL。',
  '- 不允许 `PHASES` / `ENTITY_STYLE` / `ENTITY_POSITIONS` / `SCENE_CONFIG` 与 `window.__BP_SOURCE_IR__` 表达不同语义事实。',
  ...
  '- 不允许跳过 `ENTITY_STYLE` 直接 `new THREE.Mesh(geom, mat)` 散布到 scene 而不挂到具名 entity Group — 每个 visible entity *必须* 由 `buildEntity(name)` 装配,装好后挂到 `models[name]`(否则 SourceIR extractor 把这些 Mesh 当 unbound 兜底,Luna 端 entity 全 fallback 默认白方块)。',
...
var COMPACT_SYSTEM_PROMPT_HEADER = [
  ...
  '- 浏览器原生 ES2018 即可,不要 TypeScript 语法。',
  '',
  '## L0 — SourceSceneIR 语义源',
  '- 顶层声明 `window.__BP_SOURCE_IR__ = { ... }`,schemaVersion=`"source-scene-ir.v1"`,kind=`"blueprint.sourceSceneIR"`。',
  '- 顶层声明 `window.__BP_SOURCE_IR_HASH__ = "<sha256>"`;hash 对 normalized semantic payload 计算,不含 generatedAt/source/diagnostics/extraction。',
  '- 顶层声明 SourceIR preview renderer ownership:`window.__BP_SOURCE_IR_PREVIEW_RENDERER_VERSION__ = "source-ir-preview-renderer.v1"`、`window.__BP_SOURCE_IR_RENDERER_OWNS_VISUALS__ = true`、`window.__BP_SOURCE_IR_RENDERER_OWNS_PHASE_DRIVER__ = true`、`window.__BP_SOURCE_IR_VISUAL_SOURCE__ = "source-scene-ir"`。',
  '- SourceSceneIR 必含 `project` / `scene` / `entities[]` / `resources[]` / `phases[]` / `hud` / `runtimeContract`。',
  '- phase id 连续 `phase1..phaseN`;showEntities/step/gate 引用必须能解析到 entities/resources。',
  '- step.kind 白名单: move_to / collect / deliver / build / upgrade / attack / spawn / despawn / show / hide / set_tip / set_resource / set_entity_state / wait / cta_finish。',
  '- gate.kind 白名单: near_entity / resource / entity_state / entity_count / timer / compound_all / compound_any / cta_arrival。',
  '- 旧兼容变量必须从 SourceSceneIR 投影派生:`PHASES` / `ENTITY_STYLE` / `ENTITY_POSITIONS` / `SCENE_CONFIG`,不能另起一套事实。',
  '- HTML runtime 可继续用 JS/Three.js,但视觉/UI/模型/phase driver 必须由 SourceIR preview renderer 从 `window.__BP_SOURCE_IR__` 派生,不要靠函数体 side effects 让 SourceIR 猜语义。',
  '',
  '## L1 — HTML 静态入口',
  '- 顶层声明 `const PHASES = [...]`(或 var/let/`window.PHASES`)。',
  '- 每个 PHASES[i] 含字段: `id` (字符串 `phase{n}`)、`name`、`goalText`、`steps[]`;推荐补 `guideText`、`durationSec`、`showEntities[]`、`trigger{type,...}`、`plannedModuleIds[]`。',
  '- `steps[]` 必须列出该 phase 的完整目标序列,并与运行时 `setTarget(...)` / targetRing / targetHint 顺序一致;不能只写最终 trigger entity。',
  ...
  '- SourceIR renderer 必须同时挂 `window.__driveToSourcePhase = window.__driveToPhase` 的同形态源端 hook。',
  ...
  '- 必须顶层 `var models = {}` 或 const,每个 entity 写 `models[<entityName>] = g;`(`g` = `new THREE.Group()`)。这是 SourceIR 反追 entity↔mesh 的 *唯一* anchor。',
  ...
  '  - `camera`: `{ fov, position:[x,y,z], lookAt:[x,y,z], near?, far? }` 或 `null`(Option C:源 PerspectiveCamera 的 FoV/机位/朝向,须与源 `camera.position.set`/`lookAt` 字面一致)。',
  ...
  '',
  '## L9 — Mesh-op 契约(Luna source-faithful,Option C,OPTION_C_SOURCE_FAITHFUL_BUILD 灰度兜底)',
  '- *顶层* 再声明 `var meshOps = {}`(不能闭包内),key = ENTITY_STYLE keys(完全一致),value = 纯字面对象数组,每个 op 描述一个原子 mesh(无 lambda / closure / Math.random / THREE 引用)。',
  '- op 必含 `kind`(`"box"|"sphere"|"cylinder"|"cone"|"plane"|"torus"|"icosahedron"`)+ `position`(`[x,y,z]` 字面量);可选 `rotation`(欧拉 degree)/`scale`/`size`(box→[w,h,d],cylinder/cone→[rTop,rBottom,h],sphere/ico→[r],plane→[w,h],torus→[radius,tube])/`color`(0xRRGGBB)/`emissive`/`emissiveIntensity`/`metalness`/`roughness`/`opacity`。所有值必须字面量。',
  '- 推荐 helper `function emitMeshOp(name, op){ meshOps[name]=meshOps[name]||[]; meshOps[name].push(op); }`,在 `buildEntity` 内每个 `new THREE.Mesh(...)` 之后调用同步声明事实(meshOps 不替代 buildEntity)。',
  '- 兜底:`meshOps[name]` 缺失/空数组 → Luna fall back 到 `spec.entities[].visual.shape` pooled-primitive(等价老行为)。',
  '',
