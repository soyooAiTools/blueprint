...
assert.match(workerJoystickSource, /bgImg\.color = new Color\(1f, 1f, 1f, 0\.45f\);/, 'Unity joystick background should remain visible in the source HTML idle style');
assert.match(workerJoystickSource, /if \(_bgImage != null\) _bgImage\.color = new Color\(1f, 1f, 1f, visible \? 0\.55f : 0\.45f\);/, 'Unity joystick visibility toggles should preserve the idle joystick cue');
assert.match(workerUiSource, /if \(!IsMissing\(bgImage\)\) bgImage\.color = new Color\(1f, 1f, 1f, 0\.45f\);/, 'source HUD relayout must keep the joystick visible without restoring the legacy cyan background');
...
// 2026-05-24 v12：参考 Unity 交付必须真拆成独立 Manager 类，禁止 partial/runtime 马甲回潮。
{
  ...
  try {
    ...
    assert.match(deliveryPlayer, /private void EnsureRuntimeComponents\(\)/, 'Player controller should bind required prehydrated runtime components for YAML-mounted scene objects');
    assert.doesNotMatch(deliveryPlayer, /gameObject\.AddComponent\(typeof\(GMP_(?:Movement|Trigger|Interaction)Component\)\)/, 'Player must not add capability components at runtime; scene hydration should preattach them');
    assert.match(deliveryPlayer, /缺少预挂 GMP_MovementComponent/, 'Player should fail loudly when scene hydration misses MovementComponent');
...
// gate target 仍保留在 PhaseGate,但不能强行塞进首帧入画列表。
{
  ...
  try {
    ...
    assert.match(levelRuleEngine, /private GameObject mTargetRing/);
    assert.match(levelRuleEngine, /CachedSceneObject\(ref mTargetRing, "__TargetRing"\)/);
    assert.match(levelRuleEngine, /CachedSceneObject\(ref mTrailLine, "__TrailLine"\)/);
    assert.match(levelRuleEngine, /CachedSceneObject\(ref mLaserLine, "__LaserLine"\)/);
    assert.match(levelRuleEngine, /CachedTargetObject\(targetEntity\)/);
    ...
    assert.match(entityBinding, /private Text\[\] mEntityLabelTexts/);
    assert.match(entityBinding, /private Image\[\] mEntityLabelPanels/);
    assert.match(entityBinding, /private GameObject\[\] mEntityObjects/);
    assert.match(entityBinding, /private Renderer\[\]\[\] mEntityRenderers/);
    assert.match(entityBinding, /UpdateSourceRuntimeFollowers\(dt\);\s*SyncVisibleEntityLabels\(\);/);
    assert.doesNotMatch(entityBinding, /SyncVisibleEntityLabelsThrottled|mNextLabelSyncTime/);
    assert.match(entityBinding, /return 1\.75f;/);
    ...
    assert.match(entityBinding, /GameObject labelObject = GMP_SceneObjectRegistry\.Find\(mEntityLabelNames\[index\]\)/);
    assert.match(entityBinding, /labelText\.text = DisplayNameForEntity\(entityName\);[\s\S]*labelText\.enabled = (?:visible|IsVisibleValue\d*)/);
    assert.match(entityBinding, /GameObject panelObject = GMP_SceneObjectRegistry\.Find\("Panel_" \+ mEntityLabelNames\[index\]\)/);
    assert.match(entityBinding, /if \(image != null\) image\.enabled = (?:visible|IsVisibleValue\d*)/);
    assert.match(entityBinding, /GMP_UI\.PositionTextOverEntity\(FindHudCanvas\(\), labelText, image, target, LabelHeightOffset\(entityName\)\)/);
    assert.match(entityBinding, /public void SyncVisibleEntityLabels\(\)/);
    ...
    const uiTool = fs.readFileSync(path.join(__dirname, '..', 'worker', 'GFM_UI.cs'), 'utf8');
    assert.match(uiTool, /LayoutText\("Text_Tip", new Vector2\(0f, 322f\), new Vector2\(248f, 44f\), 18\)/, 'top guide pill should use source HTML width, not full-width fallback');
    assert.match(uiTool, /LayoutText\("Text_TargetHint", new Vector2\(0f, -324f\), new Vector2\(103f, 36f\), 15\)/, 'target hint should use source HTML compact pill size');
    assert.match(uiTool, /rect\.sizeDelta = LabelSizeForText\(text\)/, 'world labels should size to text like source HTML DOM labels');
    assert.match(uiTool, /text\.fontSize = 12;/, 'world label font should match source HTML DOM labels');
