...
(function emitsWhenActive() {
  ...
  // re-bind to composite root
  assert.match(code, /GameSceneCtrl\.instance\.Register\("Player", "Player"\)/, 're-binds Player to composite root');
  assert.match(code, /GameObject __root = new GameObject\("DrillStation"\)/, 'creates DrillStation root');
