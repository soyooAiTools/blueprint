...
assert.match(code, /GFM_CameraController\.Instance\.MoveTo\(__autoPlayCameraPulsePosition\);/);
assert.match(code, /mainCam\.fieldOfView = Mathf\.Clamp\(_autoPlayCameraBaseFov \+ pulse \* 3\.0f, 32f, 64f\);/);
assert.match(code, /if \(mainCam\.orthographic\) GFM_CameraController\.Instance\.SetOrthographicSize\(Mathf\.Clamp\(_autoPlayCameraBaseOrthoSize \+ pulse \* 0\.70f, 3\.5f, 14f\)\);/);
