...
(async function() {
  var dir = fs.mkdtempSync(path.join(os.tmpdir(), 'storyboard-openai-image-provider-'));
  ...
  var cli = spawnSync(process.execPath, [
    path.join(__dirname, '..', 'scripts/storyboard-openai-image-provider.cjs'),
  ], {
    cwd: path.join(__dirname, '..'),
    env: Object.assign({}, process.env, {
      STORYBOARD_OPENAI_DRY_RUN: '1',
