...
// task #52 (v1.4d-ε) — scripts/migrate-v1.3-to-v1.4d.cjs unit coverage.
// Uses tiny synthetic source HTML with a 3-phase PHASES array + minimal v1.3
// contract; no Path B harness dependency. Also covers field-diff Stage 5
// severity downgrade for plain-string pre-v1.4 contracts.
//
// Covers:
//   1. extractPhasesBlock catches `const PHASES = [...]`
//   2. parsePhases pulls `{id, guideText}` from each top-level entry
//   3. buildPolymorphicHud produces correct {perPhase: {...}} for all 3 slots
//   4. happy path: 3-phase HTML + v1.3 contract → schemaVersion bumps to 1.4.0,
...
function syntheticHtml(opts) {
  ...
  var phases = opts.phases || [
    { id: 'phase1', guideText: 'go to A' },
    { id: 'phase2', guideText: 'go to B' },
    { id: 'phase3', guideText: 'go to C' }
  ];
  var entries = phases.map(function(p) {
    return '  {id:"' + p.id + '",name:"n",guideText:"' + p.guideText + '",steps:[{target:"X",label:"l"}],trigger:{type:"t"}}';
  }).join(',\n');
...
// ─── case 3: buildPolymorphicHud ──────────────────────────────────────────────
var poly = migrateLib.buildPolymorphicHud(phases1);
...
assert.deepStrictEqual(poly['hud.targethint'], { perPhase: { phase1: '\u76ee\u6807\uff1ago to A', phase2: '\u76ee\u6807\uff1ago to B', phase3: '\u76ee\u6807\uff1ago to C' } });
...
console.log('migrate-v1.3-to-v1.4d.test.cjs PASS — 15 cases');
