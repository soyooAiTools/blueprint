...
(async function() {
  var dir = fs.mkdtempSync(path.join(os.tmpdir(), 'storyboard-codex-image-provider-'));
  ...
  var cli = spawnSync(process.execPath, [
    path.join(__dirname, '..', 'scripts/storyboard-codex-image-provider.cjs'),
  ], {
    cwd: path.join(__dirname, '..'),
    env: Object.assign({}, process.env, {
      STORYBOARD_CODEX_DRY_RUN: '1',
