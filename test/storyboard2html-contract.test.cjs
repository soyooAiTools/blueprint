...
assert.ok(contract.htmlStaticEntry.requiredGlobals.some(function(entry) { return entry.name === 'window.__BP_SOURCE_IR_RENDERER_OWNS_VISUALS__'; }));
assert.ok(contract.htmlStaticEntry.requiredGlobals.some(function(entry) { return entry.name === 'window.__BP_SOURCE_IR_RENDERER_OWNS_PHASE_DRIVER__'; }));
assert.ok(contract.htmlStaticEntry.requiredGlobals.some(function(entry) { return entry.name === 'window.__driveToSourcePhase'; }));
assert.strictEqual(contract.htmlStaticEntry.renderingSourceOfTruth, 'source-ir-preview-renderer');
