...
var skillRoot = '/opt/blueprint-skill';
...
var historicalDocs = [
  'docs/specs/2026-04-18-feedback-reform-spec.md',
  'docs/specs/2026-04-18-space-junk-refactor-spec.md',
  'docs/specs/2026-04-19-codegen-quality-systematic-spec.md',
  'docs/superpowers/specs/2026-03-18-base-template-project-design.md',
].map(function(rel) {
  return fs.readFileSync(path.join(editorRoot, rel), 'utf8');
}).join('\n\n');
...
assertHas(skill, 'N=10 cold-export corpus 已作为文件级 gate 通过', 'SKILL.md');
...
assertHas(historicalDocs, '2026-06-23 gmp-v14 legacy / unitycomponent-v1 双 profile', 'historical docs');
assertHas(historicalDocs, '显式 `unitycomponent-v1`', 'historical docs');
...
[
  '2026-06-22 UnityComponent(3) / Blueprint 2.0 程序员交付口径取代',
  '当前权威入口是 UnityComponent(3) / Blueprint 2.0',
  '最终程序员交付必须走 `Assets/Scripts/Core` / `Tool` / `Game`',
  '当前最终交付以 AIBridge/Editor hydration',
  '当前口径要求稀疏关键中文注释、Core/Tool/Game 三层',
].forEach(function(needle) {
  assertNotHas(historicalDocs, needle, 'historical docs');
});
