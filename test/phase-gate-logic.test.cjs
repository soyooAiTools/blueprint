...
try {
  ...
  const phasePreset = fs.readFileSync(path.join(scripts, 'Common', 'PhasePreset.cs'), 'utf8');
  assert.match(phasePreset, /switch \(kind\)/);
  assert.match(phasePreset, /case "timer"/);
  assert.match(phasePreset, /case "resource"/);
  assert.match(phasePreset, /case "entity"/);
  assert.match(phasePreset, /case "entity_count"/);
  assert.match(phasePreset, /GMP_EconomyManager\.instance\.GetResource\(target\)/);
  assert.match(phasePreset, /EntityBindingManager\.instance\.GetState\(target\)/);

  const phaseController = fs.readFileSync(path.join(scripts, 'Manager', 'PhaseController.cs'), 'utf8');
  assert.match(phaseController, /bool dwellReady = phaseTimer >= required \|\| phaseRealTimer >= required/);
  assert.match(phaseController, /bool gateReady = preset\.gate == null \|\| preset\.gate\.IsReady/);
  assert.doesNotMatch(phaseController, /EventRuleEngine\.instance\.IsPhaseComplete/);

  const entityBinding = fs.readFileSync(path.join(scripts, 'Manager', 'EntityBindingManager.cs'), 'utf8');
  assert.match(entityBinding, /public int GetActiveCount\(string entityName\)/);

  const phase1 = fs.readFileSync(path.join(tmp, 'Assets', 'Phases', 'Phase1.asset'), 'utf8');
  const phase2 = fs.readFileSync(path.join(tmp, 'Assets', 'Phases', 'Phase2.asset'), 'utf8');
  const phase3 = fs.readFileSync(path.join(tmp, 'Assets', 'Phases', 'Phase3.asset'), 'utf8');
  assert.match(phase1, /targetEntity: "_gold"/);
  assert.match(phase1, /kind: "entity"/);
  assert.match(phase1, /target: "_gold"/);
  assert.match(phase1, /threshold: 2/);
  assert.match(phase1, /  - _gold/);
  assert.match(phase2, /target: "_base"/);
  assert.match(phase3, /target: "_ctaButton"/);
} finally {
