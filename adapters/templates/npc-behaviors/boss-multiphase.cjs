...
function generateSystem(npc) {
  ...
  lines.push('            Mark' + npc.entity + 'Defeated();');
  lines.push('            return;');
  lines.push('        }');
  ...
  lines.push('            Update' + npc.entity + 'CombatPhase(dt, hpPercent, dist);');
  lines.push('        }');
  lines.push('    }');
  lines.push('');
  lines.push('    // Mark the ' + npc.entity + ' boss defeated and emit combat progress evidence.');
  lines.push('    void Mark' + npc.entity + 'Defeated() {');
  lines.push('            ' + v + 'State = 2;');
  lines.push('            HideObj(' + v + ');');
  lines.push('            enemiesDefeated++;');
  lines.push('    }');
  lines.push('');
  lines.push('    // Select the boss behavior branch from health percentage, then call the focused phase helper.');
  lines.push('    void Update' + npc.entity + 'CombatPhase(float dt, float hpPercent, float dist) {');
  lines.push('        if (hpPercent > 66f) {');
  lines.push('            ' + v + 'BossPhase = 1;');
  lines.push('            Update' + npc.entity + 'MeleePhase(dt, dist, ' + p.moveSpeed + 'f, 1f);');
  lines.push('        } else if (hpPercent > 33f) {');
  lines.push('            ' + v + 'BossPhase = 2;');
  lines.push('            Update' + npc.entity + 'RangedPhase(dt, dist);');
  lines.push('        } else {');
  lines.push('            ' + v + 'BossPhase = 3;');
  lines.push('            Update' + npc.entity + 'MeleePhase(dt, dist, ' + p.moveSpeed + 'f * 2f, 0.5f);');
  lines.push('        }');
  lines.push('    }');
  lines.push('');
  lines.push('    // Move the boss toward the player and apply melee damage when in range.');
  lines.push('    void Update' + npc.entity + 'MeleePhase(float dt, float dist, float speed, float cooldownScale) {');
  lines.push('        Vector3 dir = (player.transform.position - ' + v + '.transform.position).normalized;');
  lines.push('        ' + v + '.transform.position += dir * speed * dt;');
  ...
  lines.push('                ' + v + 'AttackTimer = ' + p.attackInterval + 'f * cooldownScale;');
  lines.push('            }');
  lines.push('        }');
  lines.push('    }');
  lines.push('');
  lines.push('    // Fire boss projectiles during the ranged phase and reset the fire cooldown.');
  lines.push('    void Update' + npc.entity + 'RangedPhase(float dt, float dist) {');
  lines.push('        ' + v + 'FireTimer -= dt;');
  lines.push('        // Boss ranged gate: only fire when phase-two cooldown is ready and target is in projectile range.');
