...
function generateVariables(npc) {
  ...
  lines.push('    int ' + v + 'MaxHP = ' + npc.params.hp + ';');
  lines.push('    int ' + v + 'State = 0; // 0=idle, 1=chase, 2=evade, 3=dead');
...
function generateSystem(npc) {
  ...
  lines.push('        if (' + v + 'State == 3) return;');
  lines.push('        if (' + v + ' == null) return;');
  lines.push('        if (' + v + 'HP <= 0) {');
  lines.push('            ' + v + 'State = 3;');
