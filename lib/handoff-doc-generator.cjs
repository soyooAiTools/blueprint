...
// ────────────────────────────────────────────────────────────
// 已知 Luna/WebGL staging 文件清单 + 程序员视角的职责说明。
// 最终 Unity 程序员交付框架以 UnityComponent(3) 口径为准，不能把这里的
// GameSceneCtrl/GFM_* staging 结构当作最终框架入口。
...
var GFM_FILE_GUIDE = [
  ...
  { file: 'GameSceneCtrl.cs',               role: 'Luna/WebGL staging 绑定缓存（非最终程序员框架入口）', edit: '不要在最终 Unity 交付中扩展它；正式场景引用走 AIBridge/SCENE_BAKE_PLAN/GMP_SceneEntityRefs' },
  { file: 'GFM_CameraController.cs',        role: '主相机缓存 + 正交等距视角 + 平滑构图',            edit: '镜头规则在这里，shot 切换走 FramePoint() / SetOrthographicSize() / SetCameraHeight()' },
  ...
  { file: 'GFM_Create.cs',                  role: 'legacy 几何体 helper（当前生成代码不应新增调用）', edit: '不要作为新对象创建入口；最终交付的临时 primitive 由 Editor bake 后删除' },
  ...
  { file: 'GFM_SingletonBase.cs',           role: 'legacy staging 兼容基类',                           edit: '不要照搬到程序员交付；最终 Manager 使用场景预挂 mInstance/instance' },
...
function buildHandoffMd(project, shots) {
  ...
  lines.push('');
  lines.push('当前 C# 源码属于 **Luna/WebGL staging 层**：画面用基本几何体（Cube/Sphere/Cylinder）+ 预染色 `__Pool_*` 物体表达，**不是最终程序员 Unity 框架**。最终 Unity 交付必须通过 AIBridge/Editor 把真实场景引用写入 `GMP_SceneEntityRefs` 或 serialized refs，不能把 `GameSceneCtrl` / raw pool literal 当第二入口。');
  lines.push('');
  ...
  lines.push('| 6 | GameSceneCtrl.cs | Luna/WebGL staging 绑定缓存；最终 Unity 交付不要围绕它改框架 |');
  lines.push('| 其余 | GFM_*.cs | 工具/Manager 库，详见 §5 |');
  lines.push('');
  lines.push('推荐阅读路径：先看 `STORYBOARD.md` 确认每个 shot 要表达什么，再回到 `GameFlowManagerMain.cs` 搜 `// Shot N`。每个 shot 按 `Phase_<id>_Init`（入画/引导）→ `Phase_<id>_OnTap`（真人点击）→ `Phase_<id>_OnAutoPlayArrive`（自动演示）→ `CheckEventRules`（退出条件）的顺序读。');
  lines.push('');
  ...
  lines.push('- 每个 shot 切换镜头走 `GFM_CameraController.Instance.FramePoint(targetPos, orthoSize)`；只缩放时用 `SetOrthographicSize`，只调高度时用 `SetCameraHeight`');
  lines.push('- 镜头平滑过渡时长（Wave 3 后启用）：约 3 秒');
  lines.push('- 入画保证：每个 shot 的 `phase.activate` 列出的实体都应在画面内；运行态 `offscreenEntities` 非空时，优先检查该 shot 的摆位和镜头目标');
  lines.push('');
  lines.push('### 4.2 物体移动');
  ...
  lines.push('');
  lines.push('### 4.5 运行态自检');
  lines.push('- `gameObject.name` 里会输出 `currentPhase`、`entityStates`、`cameraState`、`offscreenEntities`、`phaseEvidence`，用于 preview/CUA 定位问题');
  lines.push('- `cameraState.offscreenCount > 0` 或 `offscreenEntities` 非空，表示有可见实体跑到镜头外，需要先修摆位或镜头构图');
  lines.push('');
  lines.push('## 5. Luna staging 工具库索引');
  lines.push('');
  lines.push('> 这一节只解释当前 WebGL staging 源码。最终程序员 Unity 交付以 UnityComponent(3) 为准：`Core/Base` + `GMP_BaseComponent` + `GMP_BaseGameFlowEntity` + `GMP_EntityManager`，场景引用走 `GMP_SceneEntityRefs` / serialized refs，短生命周期对象走 `GMP_Pool`。');
  lines.push('');
  ...
  lines.push('## 6. 二期接入清单');
  lines.push('');
  lines.push('1. **替换美术资产**：最终 Unity 程序员交付不要改 `GameSceneCtrl.cs`。先更新 `SCENE_BAKE_PLAN.json` / Editor bake，再把真实 prefab、Mesh 和关键场景对象写入 `GMP_SceneEntityRefs` 或 serialized refs');
  ...
  lines.push('');
  lines.push('## 6.1 流程增删改指南');
  lines.push('');
  lines.push('- **修改流程**：先在 `STORYBOARD.md` 找到对应 Shot，再改 `GameFlowManagerMain.Flow.cs` 中的 `Phase_<id>_Init`、交互处理和退出条件；只改文案时优先改该 phase 的 guide/引导文本。');
  lines.push('- **删除流程**：删除 storyboard phase 后，同步移除对应 `Phase_<id>_*` 方法、退出条件和自动播放目标；不要用跳过整段 Update 或重置全场来绕过。');
  lines.push('- **增加流程**：新增 storyboard phase，补 `Phase_<id>_Init`、真人交互入口、自动播放入口和退出条件，再把它插入现有 phase 顺序。');
  lines.push('- **例子**：要在 `collectScrap` 后增加 `upgradeShip`，先新增 `upgradeShip` phase 和引导文案，再实现 `Phase_upgradeShip_Init`，设置目标实体 `_shipUpgrade`，最后让上一阶段完成后进入 `upgradeShip`。');
  lines.push('- 改完后重新跑交付校验，确认代码关系图、分镜对应表和实际 C# 文件仍一致。');
  lines.push('');
  lines.push('## 7. 反馈与迭代');
  lines.push('');
...
function buildStoryboardMd(project, shots) {
  ...
  shots.forEach(function(s) {
    ...
    if (s.phaseId) {
      lines.push('**代码阅读顺序**：`Phase_' + s.phaseId + '_Init` → `Phase_' + s.phaseId + '_OnTap` → `Phase_' + s.phaseId + '_OnAutoPlayArrive` → `CheckEventRules`');
      lines.push('');
    }
    lines.push('---');
    lines.push('');
