...
function replacePhrases(text) {
  ...
  var replacements = [
    ...
    [/Generated from storyboard spec\. AI fills TODO sections only\./g, '由故事板规格生成。TODO 区域只补业务逻辑。'],
    ...
    [/NEVER call GFM_Create\.Obj\(\) — pool objects already exist, use GameObject\.Find\(\)/g, '禁止调用 GFM_Create.Obj()；Luna staging 用骨架绑定字段、RegisterEntityBindings() 或 GameSceneCtrl.instance.Get()，程序员交付用 GMP_SceneEntityRefs/serialized refs'],
    ...
    [/AI fills — place additional objects, set colors, show guide/g, '补充：摆放额外物体、设置颜色、显示引导'],
    [/AI fills — activate objects for (.*)/g, '激活 $1 所需物体'],
    ...
    [/Phase "([^"]+)" tap handler\. AI\/template fills TODO region\./g, '阶段 "$1" 的点击处理器。TODO 区域补充交互逻辑。'],
    [/AI\/template fills — produce observable movement or other real gameplay progress here\./g, '在这里产生可观察位移或其他真实玩法进度。'],
    ...
    [/AI fills — move\/activate entities so EntityAdvanced\(\.\.\.\) becomes true/g, '移动或激活实体，让 EntityAdvanced(...) 变为 true'],
    ...
    [/when AI leaves the phase handler empty\. This mutates both transform/g, '当阶段处理器留空时，本回退会同时修改 transform'],
    ...
    [/Entity states — must reach terminal state/g, '实体状态：仅给模板/UI 同步使用。约定 0=未激活/等待，1=处理中，2=完成/可见。'],
    [/Anti-autoplay flags — AI MUST set these to true when player performs the required interaction/g, '交互标记：记录玩家/CUA 是否触发过操作；phase 出口仍以真实位移或 evidence 为准。'],
    [/Set to true on player interaction \(click\/drag\/joystick\)/g, '玩家交互时置为 true'],
    ...
    [/Entity variable shortcuts \(backed by GameSceneCtrl cache\)/g, '实体变量快捷引用（Luna staging 绑定缓存支持，非程序员最终框架入口）'],
    ...
    [/AI fills _resources array in Start\(\); skeleton syncs it to Manager once\./g, 'Start() 中填充 _resources 数组后，骨架会同步一次到 Manager。'],
    ...
    [/GameSceneCtrl\.cs — scene entity management singleton/g, 'GameSceneCtrl.cs：Luna staging 场景绑定缓存（非程序员最终框架入口）'],
    [/Centralizes Find\/cache\/show\/hide to reduce skeleton boilerplate\./g, '集中处理 staging 绑定缓存/显示/隐藏，减少骨架样板代码；最终程序员交付走 GMP_SceneEntityRefs。'],
    ...
    [/Fallback: CreatePrimitive \(won't render in Luna, but compiles\)/g, '旧兜底：CreatePrimitive 在当前交付中禁止使用，应改为预绑定对象或 Editor 烘焙资源'],
    [/Hide all pool objects \(move offscreen\)/g, '隐藏所有对象池物体（移出屏幕）'],
    [/Auto-assign distinct base color by primitive type \(fallback if AI doesn't SetColor\)/g, '按 primitive 类型自动分配不同基础色（未 SetColor 时的回退）'],
    [/brown/g, '棕色'],
    [/green/g, '绿色'],
    [/steel gray/g, '钢灰色'],
    [/dark brown/g, '深棕色'],
...
function localizeFallback(text) {
  ...
  if (/^\[(?:SKELETON|AUTO-REPAIR|REMINDER|IMMUTABLE|ASSEMBLY|LUNA|CUA)\b/.test(trimmed)) return trimmed;
...
function localizeCommentBody(body) {
  ...
  if (/^ok$/i.test(trimmed)) return original;
  if (/^\[ASSEMBLY (?:OWNER MANIFEST|PHASE|SLOT)\]/.test(trimmed)) return original;

  var translated = replacePhrases(text);
  if (/^\s*"[^"]+"\s*:/.test(translated)) {
    translated = translated.replace(/^(\s*)/, '$1数据: ');
  }
  ...
  translated = translated
    ...
    .replace(/=== TODO: AI 填充：激活 (.*) === 所需物体/g, '=== TODO：激活 $1 所需物体 ===');
