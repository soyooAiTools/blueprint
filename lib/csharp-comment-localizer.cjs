...
function replacePhrases(text) {
  ...
  var replacements = [
    ...
    [/NEVER call GFM_Create\.Obj\(\) — pool objects already exist, use GameObject\.Find\(\)/g, '禁止调用 GFM_Create.Obj()；对象引用必须来自骨架绑定字段、RegisterEntityBindings() 或 GameSceneCtrl.instance.Get()'],
