...
function assetBindingSourceString(value) {
...
function validateGeneratedDeliveryData(root, errors, spec) {
  ...
  (spec.phases || []).forEach(function(phase) {
    ...
    if (!hasQuotedValue(text, stableJsonString(phase.gate || {}))) {
      add(errors, 'generated-phase-data-mismatch', 'GeneratedDeliveryData.Phases must include every spec phase gate as stable JSON', { phaseId: phase.id, key: 'gate' });
    }
    if (!hasQuotedValue(text, stableJsonString(phase.steps || []))) {
      add(errors, 'generated-phase-data-mismatch', 'GeneratedDeliveryData.Phases must include every spec phase step list as stable JSON', { phaseId: phase.id, key: 'steps' });
    }
  });
  (spec.entities || []).forEach(function(entity) {
  ...
  (spec.resources || []).forEach(function(resource) {
    ['id', 'label', 'kind'].forEach(function(key) {
      if (!hasQuotedValue(text, resource[key] || '')) {
        add(errors, 'generated-resource-data-mismatch', 'GeneratedDeliveryData.Resources must include every spec resource field', { resource: resource.id, key: key });
      }
    });
  });
  ...
  (spec.assetBindings || []).forEach(function(item) {
    ...
    if (!hasQuotedValue(text, assetBindingSourceString(item.source))) {
      add(errors, 'generated-asset-binding-data-mismatch', 'GeneratedDeliveryData.AssetBindings must include a stable serialized source for every asset binding', { assetId: item.assetId, key: 'source' });
    }
  });
  if (text.indexOf('[object Object]') >= 0) {
    add(errors, 'assetbindings-source-invalid', 'GeneratedDeliveryData.AssetBindings.Source must be stable JSON/string data, not JavaScript object coercion');
  }
}
...
function validateSpecConsistency(root, errors) {
  ...
  ['entities', 'components', 'phases', 'resources', 'sceneRefs', 'poolArchetypes', 'uiRefs', 'assetBindings'].forEach(function(key) {
