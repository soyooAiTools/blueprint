...
var REQUIRED_SCENE_SCRIPTS = [
  ...
  { rel: 'Assets/Scripts/Core/Modules/GMP_EntityManager.cs', objectName: 'GMP_EntityManager' },
...
function validateRequiredSceneScripts(root, scene, scripts, errors) {
  ...
  REQUIRED_SCENE_SCRIPTS.forEach(function(item) {
    if (item.alternatives) {
      details.push(validateRequiredSceneScriptAlternative(root, scene, scripts, item.alternatives, errors));
      return;
    }
    details.push(validateRequiredSceneScript(root, scene, scripts, item, errors));
  });
  return details;
}

function validateRequiredSceneScriptAlternative(root, scene, scripts, alternatives, errors) {
  var localErrors = [];
  var candidates = alternatives.map(function(item) {
    return validateRequiredSceneScript(root, scene, scripts, item, localErrors);
  });
  var passed = candidates.some(function(item) { return item.status === 'ok'; });
  if (!passed) {
    localErrors.forEach(function(error) { errors.push(error); });
  }
  return {
    objectName: alternatives.map(function(item) { return item.objectName; }).join(' or '),
    script: alternatives.map(function(item) { return item.rel; }).join(' or '),
    expectedGuid: '',
    actualGuids: [],
    status: passed ? 'ok' : 'missing_alternative',
    alternatives: candidates
  };
}

function validateRequiredSceneScript(root, scene, scripts, item, errors) {
