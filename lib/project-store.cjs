...
const { normalizeProjectBlueprint } = require('./project-blueprint-normalizer.cjs');
...
function readProject(id) {
  ...
  return normalizeProjectBlueprint(JSON.parse(fs.readFileSync(filePath, 'utf-8')));
}

function writeProject(project) {
  project = normalizeProjectBlueprint(project);
...
function listProjects() {
  ...
  files.forEach(function(f) {
    try {
      var data = normalizeProjectBlueprint(JSON.parse(fs.readFileSync(path.join(PROJECTS_DIR, f), 'utf-8')));
