...
function buildMarkdown(info) {
  ...
  var structureNote = isBaseLayerDelivery
    ? '当前是程序员交付结构：`GameFlowManagerMain.cs` 是 Unity 挂载入口，`GameFlow*Base.cs` 按流程控制、输入处理、资源系统、UI 和场景控制分层，每个 GameFlow 脚本应保持在 1000 行以内。'
    : (info.hasManagerPartials
  ...
  var uiNode = isBaseLayerDelivery
    ? 'GameFlowStateBase.cs<br/>实体绑定 / UI helper / 场景辅助'
