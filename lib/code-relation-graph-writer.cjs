...
function collectV14PhaseNames(root) {
  ...
  return fs.readdirSync(dir)
    .filter(function(name) { return /^Phase\d+\.asset$/i.test(name); })
    .sort(function(a, b) {
...
function buildMarkdown(info) {
  ...
  if (isV14Delivery) {
    coreRows = [
      ...
      '| `Assets/Scripts/Game/Phases` | PhasePreset 资产，gate kind 使用 enum 数值 |'
    ];
...
function buildHtml(info) {
  ...
  var cards = isV14Delivery ? [
    ...
    '<div class="card warn"><strong>Game/Phases</strong><span>PhasePreset assets / enum gates</span></div>'
  ] : (isBaseLayerDelivery ? [
