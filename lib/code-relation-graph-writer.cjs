...
function collectProjectInfo(root, options) {
  ...
  var mainFileName = byName['GMP_MainManager.cs'] ? 'GMP_MainManager.cs' : (byName['MainManager.cs'] ? 'MainManager.cs' : 'GameFlowManagerMain.cs');
  var mainText = readText(byName[mainFileName]);
  var baseLayerNames = [
...
function buildMarkdown(info) {
  ...
  var structureNote = isBaseLayerDelivery
    ...
    : (isV14Delivery
    ? '当前是 v14 程序员交付结构：`Assets/Scripts` 顶层只分 `Core` / `Tool` / `Game`；每个游戏只在 `Game` 里写业务代码，Core/Tool 承载可复用基础能力。'
    : (info.hasManagerPartials
  ...
  return [
    ...
    '    Commons["Common(s)/GFM_*.cs<br/>通用库"]',
    '    Main --> Entities',
    '    Main --> Flow',
    '    Flow --> Domain',
    '    Main --> Resource',
...
function buildHtml(info) {
  ...
  var structureLabel = isBaseLayerDelivery
    ...
    : (isV14Delivery
      ? 'v14 程序员交付版：Assets/Scripts 顶层只保留 Core / Tool / Game，具体项目业务集中在 Game。'
      : (info.hasManagerPartials
