...
function collectV14PhaseNames(root) {
  var dir = path.join(root, 'Assets', 'Scripts', 'Game', 'Phases');
  if (!fs.existsSync(dir) || !fs.statSync(dir).isDirectory()) return [];
  return fs.readdirSync(dir)
    .filter(function(name) { return /\.asset$/i.test(name); })
    .sort(function(a, b) {
      var orderA = (/^(?:Flow|Phase)(\d+)/i.exec(a) || [])[1];
      var orderB = (/^(?:Flow|Phase)(\d+)/i.exec(b) || [])[1];
      if (orderA !== orderB) return Number(orderA || Number.MAX_SAFE_INTEGER) - Number(orderB || Number.MAX_SAFE_INTEGER);
      return a.localeCompare(b);
    })
    .map(function(name) {
      var text = readText(path.join(dir, name));
      var idMatch = /\n  mPhaseId:\s*([^\n\r]*)/.exec(text);
      if (idMatch && String(idMatch[1]).trim()) return String(idMatch[1]).trim().replace(/^"|"$/g, '');
      var nameMatch = /\n  m_Name:\s*([^\n\r]*)/.exec(text);
      if (nameMatch && String(nameMatch[1]).trim()) return String(nameMatch[1]).trim().replace(/^"|"$/g, '');
...
function collectProjectInfo(root, options) {
  ...
  var hasV14Layout = fs.existsSync(path.join(root, 'Assets', 'Scripts', 'Core'))
    && fs.existsSync(path.join(root, 'Assets', 'Scripts', 'Tool'))
    && fs.existsSync(path.join(root, 'Assets', 'Scripts', 'Game'));
  var mainFile = byName['GMP_MainManager.cs'] || byName['MainManager.cs'] || byName['GameFlowManagerMain.cs'];
  var mainFileName = byName['GMP_MainManager.cs']
    ? (hasV14Layout ? relative(root, byName['GMP_MainManager.cs']).replace(/^Assets\/Scripts\//, '') : 'GMP_MainManager.cs')
    : (byName['MainManager.cs'] ? 'MainManager.cs' : 'GameFlowManagerMain.cs');
  var mainText = readText(mainFile);
  var baseLayerNames = [
  ...
  var phases = hasV14Layout ? collectV14PhaseNames(root) : Object.keys(phaseSet);
  ...
  var hasCommons = fs.existsSync(path.join(root, 'Assets', 'Program', 'Script', 'Commons'))
    || fs.existsSync(path.join(root, 'Assets', 'Scripts', 'Common'))
    || fs.existsSync(path.join(root, 'Assets', 'Scripts', 'Commons'))
    || fs.existsSync(path.join(root, 'Assets', 'Script', 'Common'))
    || fs.existsSync(path.join(root, 'Assets', 'Script', 'Commons'))
  ...
  return {
    ...
    hasV14Layout: hasV14Layout,
    hasReferenceMainManager: mainFileName === 'MainManager.cs',
...
function buildMarkdown(info) {
  ...
  var entityText = info.entityClassFiles.length
    ? ((isV14Delivery ? 'Game/Entities/*.cs' : 'Entities/*.cs') + ' (' + info.entityClassFiles.length + ' files)')
    : 'GameObject 字段';
  var structureNote = isBaseLayerDelivery
    ...
    : (isV14Delivery
    ? '当前是 Blueprint 2.0 程序员交付结构：`Assets/Scripts` 顶层只分 `Core` / `Tool` / `Game`；每个游戏只在 `Game` 里写业务代码，Core/Tool 承载可复用基础能力。'
    : (info.hasManagerPartials
    ? '当前是程序员交付结构：`GameFlowManagerMain` 拆为 Main + Flow/Input/Resource/UI/Scene 5 个 partial，按职责分别维护。'
    : '当前是程序员交付结构：`' + mainFileName + '` 是普通类，实体领域属性在 `Entities/` 下维护。'));
  ...
  var flowNode = isV14Delivery
    ? 'Core/Modules/GMP_PhaseController.cs<br/>阶段流 / enum gate'
    : (isBaseLayerDelivery
  ...
  var resourceNode = isV14Delivery
    ? 'Core/Modules + Core/Components<br/>Pool / Audio / Event / Economy / UI'
    : (isBaseLayerDelivery
    ? (hasLayer('GameFlowResourceBase.cs')
        ? 'GameFlowResourceBase.cs<br/>资源 / 经济 / 背包'
        : (hasLayer('GameFlowPreviewBase.cs')
            ? 'GameFlowPreviewBase.cs<br/>Preview / evidence'
            : '资源 / Preview<br/>本次导出未启用'))
    : 'Resource.cs<br/>资源采集 / 花费 / 回收');
  var uiNode = isV14Delivery
    ? 'Tool + Core/Components<br/>相机 / UI / 摇杆 / 移动 / 触发'
    : (isBaseLayerDelivery
  ...
  var sceneInputNode = isV14Delivery
    ? 'Game/Level/GMP_LevelRuleEngine.cs<br/>关卡业务 / 交互规则'
    : (isBaseLayerDelivery
  ...
  var autoPlayFieldName = info.hasReferenceMainManager ? 'mAutoPlayMode' : '_autoPlayMode';
  var commonsNode = isV14Delivery
    ? 'Core/Common/GMP_*.cs<br/>ResourceIds / Activator'
    : 'Common(s)/GFM_*.cs<br/>通用库';
  var runtimeLines = isV14Delivery ? [
    'flowchart TD',
    '    Start["Start()"] --> Boot["GMP_MainManager.InitCoreModules"] --> PhaseStart["GMP_PhaseController.StartFlow / EnterPhase"]',
    '    Update["Update()"] --> PhaseTick["GMP_PhaseController.Tick"]',
    '    PhaseTick --> Rules["GMP_LevelRuleEngine.OnEnterPhase / Tick / TryInteract"]',
    '    Rules --> Mode{"mAutoPlayMode ?"}',
    '    Mode -->|是| AutoTick["GMP_AutoPlayDriver.Tick / OnArrive"]',
    '    Mode -->|否| PlayerTick["GMP_Player.Tick / Trigger / Interaction"]',
    '    AutoTick --> Systems["GMP_SceneEntityRefs / Economy / HUD"]',
    '    PlayerTick --> Systems',
    '    Systems --> Export["Phase gate / MarkSuccess / CTA"]'
  ].join('\n') : [
    'flowchart TD',
  ...
  var coreRows = [
    '| `' + mainFileName + '` | Unity 生命周期入口、主 `Update()`、阶段跳转出口 |'
  ];
  if (isV14Delivery) {
    coreRows = [
      '| `Assets/Scripts/Core/Base` | 核心枚举、实体/玩家/NPC 基类；管理器不用 MonoSingleton |',
      '| `Assets/Scripts/Core/Components` | Movement、Trigger、Interaction、Inventory、Skill 等可复用组件 |',
      '| `Assets/Scripts/Core/Modules` | 主管理器、Pool、Audio、Event、Phase、Economy、UI 等核心模块 |',
      '| `Assets/Scripts/Core/Common` | ResourceIds、ScriptActivator 等通用工具 |',
      '| `Assets/Scripts/Tool` | 相机、UI、视觉引导等跨项目工具 |',
      '| `Assets/Scripts/Game/Level` | 关卡流程、业务事件名和交互规则 |',
      '| `Assets/Scripts/Game/Entities` | 本游戏具体实体类，直接继承 Core 基类 |',
      '| `Assets/Scripts/Game/Player` | 本游戏 Player 控制器，按需挂 Core 组件 |',
      '| `Assets/Scripts/Game/Phases` | 连续试玩流程节点资产，文件名和 `mPhaseId` 使用序号 + 业务语义，gate kind 使用 enum 数值 |'
    ];
  } else if (isBaseLayerDelivery) {
    var layerDescriptions = {
      ...
      'GameFlowPreviewBase.cs': 'Preview/CUA 状态导出',
      'GameFlowResourceBase.cs': '资源、经济、背包和形态系统',
  ...
  if (!isV14Delivery) {
    coreRows = coreRows.concat([
      '| `Entities/BaseGameFlowEntity.cs` | 实体通用绑定、命名、可见性和位移动画 |',
      '| `Entities/*Entity.cs` | 面向对象实体模型，绑定对象池 `GameObject` 后承载领域状态 |',
      '| `Common(s)/GFM_*.cs` | 通用工具库；业务逻辑优先写在主流程或实体领域类中 |'
    ]);
  }
  var reminders = isV14Delivery ? [
    '- ' + structureNote,
    '- Core 只放可复用基础能力，不写具体关卡事件名或一次性玩法规则。',
    '- 场景实体引用从 `Game/Level/GMP_SceneEntityRefs.cs` 读起；新增实体维护显式字段，不新增通用 object binding 表。',
    '- 关卡业务从 `Game/Level/GMP_LevelRuleEngine.cs` 读起；新增玩法优先扩展 `Game`，只有通用能力才下沉 Core/Tool。',
    '- Player 按需选择 Movement/Trigger/Interaction/Inventory/Skill 等组件，不强行继承多层业务基类。',
    '- Phase gate 使用 `GMP_PhaseGateKind` enum，禁止回退到字符串 `"timer"` / `"entity"`。'
  ] : [
    '- ' + structureNote,
    '- 实体引用只来自 serialized refs 或 `GMP_SceneEntityRefs`，不要在业务代码里二次 `GameObject.Find("__Pool_*")` 覆盖。',
    '- 新增建筑、兵营、炮塔等对象时，优先新增/扩展 `Entities/` 下的具体类，不要再用主流程文件分类表达领域对象。',
    '- 资源 ID 使用 `GFM_ResourceIds`，不要裸写 `"Gold"` / `"gold"`。',
    '- guide 文案统一通过 `SetGuideText()`。'
  ];
  if (!isBaseLayerDelivery && !isV14Delivery) {
  ...
  return [
    ...
    '```mermaid',
    'flowchart TD',
    '    Main["' + mainFileName + '<br/>Start / Update / 阶段协调"]',
...
function buildHtml(info) {
  ...
  var entityLabel = info.entityClassFiles.length
    ? (isV14Delivery ? 'Game/Entities/*.cs' : 'Entities/*.cs')
    : '实体字段';
  var structureLabel = isBaseLayerDelivery
    ? '程序员交付版：GameFlowManagerMain 是普通入口，GameFlow*Base.cs 用普通继承链分层，实体领域属性在 Entities/。'
    : (isV14Delivery
      ? 'Blueprint 2.0 程序员交付版：Assets/Scripts 顶层只保留 Core / Tool / Game，具体项目业务集中在 Game。'
      : (info.hasManagerPartials
      ? 'Blueprint 审核源可能仍有 companion 文件；程序员交付版会合并为普通类。'
      : '程序员交付版：' + mainFileName + ' 是普通类，实体领域属性在 Entities/。'));
  var cards = isV14Delivery ? [
    '<div class="card main"><strong>Core/Base</strong><span>Core enums / entity base / no MonoSingleton</span></div>',
    '<div class="card main"><strong>Core/Modules</strong><span>Main / Phase / Event / UI / Economy</span></div>',
    '<div class="card flow"><strong>Core/Components</strong><span>Movement / Trigger / Interaction / Inventory / Skill</span></div>',
    '<div class="card"><strong>Core/Common</strong><span>Scene registry / resource ids / activator</span></div>',
    '<div class="card"><strong>Tool</strong><span>Camera / UI / visual guide</span></div>',
    '<div class="card flow"><strong>Game/Level</strong><span>Level rules / event names / bindings</span></div>',
    '<div class="card data"><strong>' + escapeHtml(entityLabel) + '</strong><span>本游戏具体实体类</span></div>',
    '<div class="card data"><strong>Game/Player</strong><span>本游戏 Player + 组件选择</span></div>',
    '<div class="card warn"><strong>Game/Phases</strong><span>semantic flow assets / enum gates</span></div>'
  ] : (isBaseLayerDelivery ? [
    ...
    '<div class="card"><strong>GameFlowResourceBase.cs</strong><span>资源 / 经济 / 背包</span></div>',
    '<div class="card"><strong>GameFlowPreviewBase.cs</strong><span>Preview / CUA 状态</span></div>',
    ...
    '<div class="card warn"><strong>Common(s)/GFM_*.cs</strong><span>通用库，业务优先写在主流程或实体类</span></div>'
  ] : [
    '<div class="card main"><strong>' + escapeHtml(mainFileName) + '</strong><span>Start / Update / 主状态</span></div>',
    '<div class="card data"><strong>' + escapeHtml(entityLabel) + '</strong><span>实体领域类 / 对象池绑定</span></div>',
  ...
  var readOrder = isBaseLayerDelivery
    ...
    : (isV14Delivery
      ? '阅读顺序：先看 <code>Core/Modules/GMP_MainManager.cs</code> 和 <code>Core/Modules/GMP_PhaseController.cs</code>，再看 <code>Game/Level</code> 与 <code>Game/Entities</code>。'
      : '阅读顺序：先看 <code>' + escapeHtml(mainFileName) + '</code> 的 <code>Start()</code> 和 <code>Update()</code>，再进入 <code>Entities/</code>。');
  var runtimeHtml = isV14Delivery
    ? '<div class="phase">Start()</div><span class="arrow">→</span><div class="phase">GMP_MainManager.InitCoreModules</div><span class="arrow">→</span><div class="phase">GMP_PhaseController.Tick</div><span class="arrow">→</span><div class="phase">GMP_LevelRuleEngine</div><span class="arrow">→</span><div class="phase">GMP_Player / AutoPlay</div><span class="arrow">→</span><div class="phase">Entity / Economy / HUD</div><span class="arrow">→</span><div class="phase">MarkSuccess / CTA</div>\n'
