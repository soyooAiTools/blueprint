...
function splitMergedManagerIntoDeliveryClasses(managerDir, mergedCode) {
  ...
  var files = {
    ...
    'GameFlowPreviewBase.cs': classCode('GameFlowPreviewBase', 'GameFlowUiBase', previewLines, '// Preview/CUA 状态导出和资源 API 门面。'),
...
function rewriteReadmeForProgrammerDelivery(root) {
  ...
  var boundary = [
    ...
    '- 输入、资源、预览、场景、UI、AutoPlay 和 Phase handler 通过普通 `GameFlow*Base.cs` 继承链分层，每个脚本保持在 1000 行以内。',
...
function writeHandoff(root, project, summary) {
  ...
  var lines = [
    ...
    '',
    '## 后续维护建议',
    '- 玩法主流程从 `GameFlowManagerMain.cs` 的 `Start()`、`Update()`、`CheckEventRules()` 开始阅读，再沿普通基类进入 `GameFlowPhase*Base`、`GameFlowUi/Input/Runtime/SceneBase` 和 `GameFlowStateBase`。',
