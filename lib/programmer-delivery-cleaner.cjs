...
function buildPhaseControllerCode() {
  return [
    ...
    '        lastRealClock = now;',
    '        if (EventRuleEngine.instance != null && EventRuleEngine.instance.IsPhaseComplete(CurrentPhase, phaseTimer, phaseRealTimer))',
    '        {',
    '            AdvancePhase();',
    '        }',
    '    }',
...
function buildAutoPlayDriverCode() {
  return [
    ...
    '        string[] targets = new string[preset.spawnEntities.Count];',
    '        for (int i = 0; i < preset.spawnEntities.Count; i++) targets[i] = preset.spawnEntities[i];',
    '        GMP_AutoPlay.instance.SetTargets(targets);',
    '    }',
    '',
    '    public void Tick(float dt)',
    '    {',
...
function buildEventRuleEngineCode() {
  return [
    ...
    '        if (phaseTimer < required && phaseRealTimer < required) return false;',
    '        if (string.IsNullOrEmpty(preset.targetEntity)) return true;',
    '        if (phaseActionDone) return true;',
    '        return EntityBindingManager.instance != null && EntityBindingManager.instance.GetState(preset.targetEntity) >= 2;',
    '    }',
    '',
