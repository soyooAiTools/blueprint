...
function findMethodRange(lines, methodName) {
  ...
  // 反馈 01 #1 (Wave B):把方法声明上方紧邻的 // /// 注释行也圈进 range,
  // 这样 partial 拆分后注释跟着方法走,不会被遗弃在原文件里。
  // 只回溯连续注释行(空行打断回溯),避免把上一段方法的尾注释抢过来。
  while (start > 0) {
    var prev = String(lines[start - 1] || '').trim();
    if (prev === '') break;
    if (prev.indexOf('//') !== 0) break;
    // 跳过 [ASSEMBLY *] / TODO_* 这类机器标记,它们由 cleaner 单独处理,不属于注释
    if (/\b(?:ASSEMBLY|TODO_)/.test(prev)) break;
    start--;
  }
  var depth = 0;
  var started = false;
...
function extractMethodsMatchingFromLines(lines, predicate) {
  ...
  for (var i = 0; i < lines.length;) {
    ...
    if (name && predicate(name, lines[i])) {
      ...
      // findMethodRange 现在会把方法上方紧邻的 // 注释纳入 range,所以 range[0] 可能 < i。
      // 只要 i 在 range 区间内,就说明我们正在处理这个方法,可以提走。
      if (range && i >= range[0] && i < range[1]) {
        if (out.length) out.push('');
        out = out.concat(extractRangeFromLines(lines, range));
        // splice 之后 lines 缩短;从 range[0] 继续,避免跳过紧跟的下一段。
        i = range[0];
        continue;
      }
    }
...
function splitMergedManagerIntoDeliveryClasses(managerDir, mergedCode) {
  ...
  var chain = [
    ...
    { fileName: 'GameFlowResourceBase.cs', className: 'GameFlowResourceBase', body: resourceLines, desc: '// 资源、经济、背包和形态系统。' },
    { fileName: 'GameFlowPreviewBase.cs', className: 'GameFlowPreviewBase', body: previewLines, desc: '// Preview/CUA 状态导出。' },
...
// Wave C (反馈 4/5)：保留 GameFlowManagerMain 的 5 个 partial（Main + Flow/Input/Resource/UI/Scene）
// 作为交付结构。不再合并到单一类，也不再生成 GameFlow*Base.cs 横切继承链——架构图明令禁止。
// 每个 partial 仍单独清理 [ASSEMBLY] 死代码,实体绑定方法落到 Main 文件以贴近 Start()。
...
function rewriteReadmeForProgrammerDelivery(root) {
  ...
  text = text.replace(
    ...
    '- Assets/Program/Script/Manager/  — GameFlowManagerMain.cs 主入口与 Flow/Input/Resource/UI/Scene partial 拆分,以及 Entities/ 面向对象实体类'
  );
  if (/## 程序员交付边界\n/.test(text)) {
...
function writeHandoff(root, project, summary) {
  ...
  var lines = [
    ...
    '- GameFlowManagerMain.cs 行数：' + mainLines,
    '- 最长 partial 文件行数：' + maxPartialLines,
