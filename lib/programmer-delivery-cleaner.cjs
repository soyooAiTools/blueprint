...
function collectEntityInfos(code) {
  ...
  while ((match = re.exec(String(code || '')))) {
    ...
    out.push({
      ...
      modelField: 'm' + entityClassName(name) + 'Model',
...
function removeRuntimeSceneObjectCreation(code) {
  ...
  text = replaceOutsideStringLiterals(text, function(segment) {
    return segment
      ...
      .replace(/\bnew\s+GameObject\s*\(/g, 'GFM_SceneObjectRegistry.Find(')
...
function buildSceneObjectRegistryCode() {
  return [
    ...
    '/// </summary>',
    'public static class GFM_SceneObjectRegistry',
    '{',
    '    /// <summary>',
...
function cleanupReferenceDeliveryCode(code) {
  return String(code || '')
    ...
    .replace(/\bGFM_ResourceIds\.m([A-Z]\w*)/g, 'GFM_ResourceIds.$1')
    .replace(/^(\s*)GFM_Luna\.Init\(gameObject\);\s*$/gm, '$1// Luna 初始化已从程序员交付版剥离。')
    .replace(/\[SKELETON\]\s*/g, '')
    .replace(/\[ASSEMBLY SIGNAL FALLBACK\]\s*/g, '')
    .replace(/TODO_PHASE_/g, 'TODO: 阶段 ')
    .replace(/TODO_/g, 'TODO: ')
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (depth === 0) {
      ...
      if (m && line.indexOf('(') < 0) {
        var oldName = m[4];
        var newName = oldName;
        if (!/^m[A-Z]/.test(oldName)) newName = 'm' + toPascalFromIdentifier(oldName);
...
function buildReferenceMainManagerCode(mainInfo, companions, entities, allCode) {
  ...
  if (!/\bGameState\s+mGameState\b/.test(body)) {
    body = '    public GameState mGameState = GameState.Start; // 当前游戏状态，Start/Run/End/Success 对齐参考工程。\n\n' + body;
  }
...
function rewriteReadmeForProgrammerDelivery(root) {
  ...
  var boundary = isReferenceMainManager ? [
    ...
    '- `Game.unity` 已预挂 `MainManager` 与关键 GFM 管理器对象，脚本物体不再由代码运行时创建。',
...
function writeHandoff(root, project, summary) {
  ...
  var lines = [
    ...
    referenceMain
      ? '- 玩法主流程从 `MainManager.cs` 的 `Start()`、`Update()`、`CheckEventRules()` 开始阅读；字段采用 `public mXxx` 命名，方便 Inspector 调整。'
...
function normalizeProgrammerDeliveryScripts(root) {
  ...
  var registryFile = path.join(scriptsRoot, 'Common', 'GFM_SceneObjectRegistry.cs');
