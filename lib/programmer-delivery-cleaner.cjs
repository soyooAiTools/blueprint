...
function phaseAssetGuids(root) {
  var dir = path.join(root, 'Assets', 'Phases');
...
function sceneObjectExtraSerializedLines(root, name) {
  if (name !== 'PhaseController') return [];
...
function collectDeliveryEntityIds(code) {
  var ids = extractStringArrayValues(code, '_entityBindingIds');
  if (ids && ids.values && ids.values.length) return ids.values.slice();
  var found = Object.create(null);
  var out = [];
  String(code || '').replace(/\bpublic\s+GameObject\s+(_[A-Za-z][A-Za-z0-9_]*)\s*;/g, function(_, name) {
    if (!found[name]) {
      found[name] = true;
      out.push(name);
    }
...
function parsePhaseDeliveryInfo(code, phaseIds) {
  var text = String(code || '');
  var out = [];
  for (var i = 0; i < phaseIds.length; i++) {
    ...
    if (visibleMatch) {
      visible = visibleMatch[1].split(',').map(function(v) { return v.trim(); }).filter(Boolean);
    }
    if (!visible.length) {
      block.replace(/PlaceObj\(\s*(_[A-Za-z][A-Za-z0-9_]*)/g, function(_, name) {
        if (visible.indexOf(name) < 0) visible.push(name);
        return _;
      });
    }
    if (visible.indexOf('_player') < 0) visible.unshift('_player');
    var target = '';
    var targetMatch = /GMP_VisualGuide\.HighlightTarget\(\s*(_[A-Za-z][A-Za-z0-9_]*)\s*\)/.exec(block);
...
function buildPhasePresetCode() {
  return [
    ...
    'using UnityEngine;',
    '',
    '[CreateAssetMenu(menuName = "GMP/PhasePreset")]',
    'public class PhasePreset : ScriptableObject',
    '{',
    '    public string phaseId; // 阶段唯一 ID,与生成流程中的 phase1/phase2 对齐。',
    '    public string guideText; // 进入阶段后显示给玩家的引导文案。',
    '    public string targetEntity; // 当前阶段主要目标实体名,为空表示不绑定单一目标。',
    '    public int targetCount; // 目标数量或进度阈值。',
    '    public string resourceKey; // 阶段关联资源 key,例如 gold/ice。',
    '    public int resourceThreshold; // 资源达成阈值。',
    '    public bool damage; // 当前阶段是否包含破坏/攻击行为。',
    '    public float minSeconds = 12f; // 阶段最短停留秒数。',
    '    public List<string> spawnEntities = new List<string>(); // 进入阶段时需要显示的实体。',
    '    public List<string> hideEntities = new List<string>(); // 进入阶段时需要隐藏的实体。',
    '    public PhaseGate gate = new PhaseGate(); // 阶段完成判定配置。',
    '}',
    '',
    '[System.Serializable]',
    'public class PhaseGate',
    '{',
    '    public string kind; // gate 类型,例如 resource/entity/timer。',
    '    public string target; // gate 目标实体或资源。',
    '    public int threshold; // gate 阈值。',
    '',
    '    public bool IsReady(float elapsedSeconds)',
    '    {',
    '        if (elapsedSeconds < 0f) return false;',
    '        string kind = this.kind == null ? "" : this.kind.Trim().ToLowerInvariant();',
    '        switch (kind)',
    '        {',
    '            case "":',
    '                return true;',
    '            case "timer":',
    '                return elapsedSeconds >= Mathf.Max(0, threshold);',
    '            case "resource":',
    '                if (string.IsNullOrEmpty(target) || GMP_EconomyManager.instance == null) return false;',
    '                return GMP_EconomyManager.instance.GetResource(target) >= Mathf.Max(1, threshold);',
    '            case "entity":',
    '                if (string.IsNullOrEmpty(target) || EntityBindingManager.instance == null) return false;',
    '                return EntityBindingManager.instance.GetState(target) >= Mathf.Max(1, threshold);',
    '            case "entity_count":',
    '                if (EntityBindingManager.instance == null) return false;',
    '                return EntityBindingManager.instance.GetActiveCount(target) >= Mathf.Max(1, threshold);',
    '            default:',
    '                return true;',
    '        }',
    '    }',
...
function buildPhaseControllerCode() {
  return [
    ...
    '{',
    '    public List<PhasePreset> phases = new List<PhasePreset>(); // Inspector 拖入的阶段资产列表。',
    '    public int currentIndex = -1; // 当前阶段索引。',
    '    public float phaseTimer = 0f; // 当前阶段运行时间。',
    '    public float phaseRealTimer = 0f; // 当前阶段真实墙钟时间。',
    '    private float lastRealClock = 0f; // 上一帧真实时间戳。',
    '    private bool flowStarted = false; // 是否已经进入第一阶段。',
    '',
    '    public PhasePreset CurrentPhase',
    '    {',
    '        get',
    '        {',
    '            if (currentIndex < 0 || currentIndex >= phases.Count) return null;',
    ...
    '        float realDt = now - lastRealClock;',
    '        if (realDt < 0f || realDt > 1f) realDt = 0f;',
    '        phaseRealTimer += realDt;',
    '        lastRealClock = now;',
    '        PhasePreset preset = CurrentPhase;',
    '        float required = AutoPlayDriver.instance != null && AutoPlayDriver.instance.isActive ? 12f : Mathf.Max(2f, preset.minSeconds);',
    '        bool dwellReady = phaseTimer >= required || phaseRealTimer >= required;',
    '        bool gateReady = preset.gate == null || preset.gate.IsReady(Mathf.Max(phaseTimer, phaseRealTimer));',
    '        if (dwellReady && gateReady)',
    '        {',
    '            AdvancePhase();',
    '        }',
    '    }',
    ...
    '        currentIndex = Mathf.Clamp(index, 0, phases.Count - 1);',
    '        phaseTimer = 0f;',
    '        phaseRealTimer = 0f;',
    '        lastRealClock = Time.realtimeSinceStartup;',
    '        PhasePreset preset = CurrentPhase;',
    '        if (EntityBindingManager.instance != null) EntityBindingManager.instance.ShowPhaseEntities(preset);',
    '        if (HudController.instance != null) HudController.instance.SetGuideText(currentIndex + 1, phases.Count, preset != null ? preset.guideText : "");',
    '        if (AutoPlayDriver.instance != null) AutoPlayDriver.instance.SetPhaseTargets(preset);',
    '        if (EventRuleEngine.instance != null) EventRuleEngine.instance.OnEnterPhase(preset);',
    '    }',
    '',
    '    public void AdvancePhase()',
    '    {',
    '        if (currentIndex + 1 >= phases.Count)',
    '        {',
    '            if (MainManager.instance != null) MainManager.instance.MarkSuccess();',
    '            return;',
    '        }',
    '        EnterPhase(currentIndex + 1);',
    '    }',
    '',
    ...
    '    {',
    '        if (HudController.instance != null) HudController.instance.SetGuideText(currentIndex + 1, phases.Count, "阶段停留过久，请检查目标实体绑定。");',
    '    }',
    '}'
  ].join('\n');
}

function buildEntityBindingManagerCode(entityIds) {
  ...
  return [
    ...
    '{',
    '    public string entityName; // 业务实体名。',
    '    public GameObject sceneObject; // 场景中拖入的实体对象。',
    '    public int initialState; // 初始状态。',
    '}',
    '',
    'public class EntityBindingManager : MonoSingleton<EntityBindingManager>',
    '{',
    '    public List<EntityBinding> bindings = new List<EntityBinding>(); // Inspector 可维护的实体绑定表。',
    '    private string[] entityNames = ' + csharpStringArrayLiteral(entityIds) + ';',
    '    private int[] entityStates; // 与 entityNames 对齐的轻量状态表。',
    '    private bool initialized = false; // 防止重复初始化。',
    '',
    '    public void Init()',
    '    {',
    ...
    '        entityStates = new int[entityNames.Length];',
    '        for (int i = 0; i < entityNames.Length; i++)',
    '        {',
    '            GameSceneCtrl.instance.Register(entityNames[i], entityNames[i]);',
    '        }',
    '        HideAllExceptPlayer();',
    '    }',
    '',
    ...
    '        Init();',
    '        Show(entityName);',
    '        SetState(entityName, Mathf.Max(1, count));',
    '    }',
    '',
    '    public GameObject Get(string entityName)',
    '    {',
    '        Init();',
    '        return GameSceneCtrl.instance != null ? GameSceneCtrl.instance.Get(entityName) : null;',
    '    }',
    '',
    ...
    '        Init();',
    '        GameObject target = Get(entityName);',
    '        if (target != null) target.transform.position = new Vector3(0f, -999f, 0f);',
    '    }',
    '',
    '    public void ShowPhaseEntities(PhasePreset preset)',
    '    {',
    '        Init();',
    '        HideAllExceptPlayer();',
    '        if (preset == null || preset.spawnEntities == null) return;',
    '        for (int i = 0; i < preset.spawnEntities.Count; i++) Show(preset.spawnEntities[i]);',
    '    }',
    '',
    ...
    '        SetState(entityName, 2);',
    '    }',
    '',
    '    public int GetState(string entityName)',
    '    {',
    '        if (entityStates == null) Init();',
    '        for (int i = 0; i < entityNames.Length; i++) if (entityNames[i] == entityName) return entityStates[i];',
    '        return 0;',
    '    }',
    '',
    ...
    '        for (int i = 0; i < entityNames.Length; i++)',
    '        {',
    '            if (!string.IsNullOrEmpty(entityName) && entityNames[i] != entityName) continue;',
    '            GameObject target = Get(entityNames[i]);',
    '            if (target != null && target.transform.position.y > -100f) count++;',
    '        }',
    '        return count;',
    '    }',
    '',
    '    private void SetState(string entityName, int state)',
    '    {',
    '        if (entityStates == null) Init();',
    '        for (int i = 0; i < entityNames.Length; i++) if (entityNames[i] == entityName) { entityStates[i] = state; return; }',
    '    }',
    '',
    ...
    '        for (int i = 0; i < entityNames.Length; i++)',
    '        {',
    '            if (entityNames[i] == "_player") continue;',
    '            Hide(entityNames[i]);',
    '        }',
    '        Show("_player");',
    '    }',
    ...
    '    {',
    '        int index = 0;',
    '        for (int i = 0; i < entityNames.Length; i++) if (entityNames[i] == entityName) { index = i; break; }',
...
function buildThinMainManagerCode() {
  return [
    ...
    'public class MainManager : MonoSingleton<MainManager>',
    '{',
    '    public GameState gameState = GameState.Start; // 当前游戏状态。',
    '',
    ...
    '        if (EntityBindingManager.instance != null) EntityBindingManager.instance.Init();',
    '        if (HudController.instance != null) HudController.instance.Init();',
    '        if (AutoPlayDriver.instance != null) AutoPlayDriver.instance.Init();',
    '        if (PhaseController.instance != null) PhaseController.instance.StartFlow();',
    '    }',
    '',
    '    void Update()',
    '    {',
    '        if (gameState == GameState.End || gameState == GameState.Success) return;',
    '        float dt = Time.deltaTime;',
    '        if (PhaseController.instance != null) PhaseController.instance.Tick(dt);',
    '        if (EntityBindingManager.instance != null) EntityBindingManager.instance.Tick(dt);',
    '        if (AutoPlayDriver.instance != null) AutoPlayDriver.instance.Tick(dt);',
    '        if (HudController.instance != null) HudController.instance.Tick(dt);',
    '        if (EventRuleEngine.instance != null) EventRuleEngine.instance.Tick(dt);',
    '    }',
    '',
    '    public void MarkSuccess()',
    '    {',
    '        gameState = GameState.Success;',
    '        if (HudController.instance != null) HudController.instance.SetGuideText(0, 0, "试玩完成，点击按钮下载。");',
    '    }',
    '}'
  ].join('\n');
}
...
function buildAutoPlayDriverCode() {
  return [
    ...
    'using UnityEngine;',
    '',
    'public class AutoPlayDriver : MonoSingleton<AutoPlayDriver>',
    '{',
    '    public bool isActive = false; // 当前是否进入自动播放。',
    '    private bool initialized = false; // 防止重复绑定回调。',
    '',
    '    public void Init()',
    '    {',
    '        if (initialized) return;',
    ...
    '    }',
    '',
    '    public void SetPhaseTargets(PhasePreset preset)',
    '    {',
    '        Init();',
    '        if (preset == null || preset.spawnEntities == null || GMP_AutoPlay.instance == null) return;',
    '        List<string> targets = new List<string>();',
    '        if (!string.IsNullOrEmpty(preset.targetEntity)) targets.Add(preset.targetEntity);',
    '        for (int i = 0; i < preset.spawnEntities.Count; i++)',
    '        {',
    '            string name = preset.spawnEntities[i];',
    ...
    '    {',
    '        if (EventRuleEngine.instance != null) EventRuleEngine.instance.HandleAutoPlayArrive(targetName);',
    '    }',
    '}'
  ].join('\n');
}
...
function buildHudControllerCode() {
  return [
    ...
    '{',
    '    public Canvas canvas; // 运行时 UI 画布。',
    '    public Text guideText; // 阶段引导文本。',
    '    public Text scoreText; // 分数文本。',
    '    private string currentGuide = ""; // 当前引导缓存。',
    '    private bool initialized = false; // 防止重复创建 UI。',
    '',
    '    public void Init()',
    '    {',
    '        if (initialized) return;',
    '        initialized = true;',
    '        canvas = GMP_UI.CreateCanvas(1920, 1080);',
    '        guideText = GMP_UI.CreateText(canvas, "", new Vector2(0, 450), 52);',
    '        scoreText = GMP_UI.CreateText(canvas, "Score: 0", new Vector2(680, 480), 40);',
    '    }',
    '',
    '    public void Tick(float dt)',
    '    {',
    ...
    '    {',
    '        Init();',
    '        currentGuide = text == null ? "" : text;',
    '        if (guideText == null) return;',
    '        string prefix = phaseIndex > 0 && totalPhases > 0 ? "[" + phaseIndex + "/" + totalPhases + "] " : "";',
    '        guideText.text = prefix + currentGuide;',
    '    }',
    '',
    '    public void SetScore(string text)',
    '    {',
    '        Init();',
    '        if (scoreText != null) scoreText.text = text == null ? "" : text;',
    '    }',
    '}'
  ].join('\n');
}

function buildEventRuleEngineCode() {
  return [
    'using UnityEngine;',
    '',
    'public class EventRuleEngine : MonoSingleton<EventRuleEngine>',
    '{',
    '    public bool phaseActionDone = false; // 当前阶段是否已经产生可观察动作。',
    '    private PhasePreset activePhase; // 当前阶段配置。',
    '',
    '    public void OnEnterPhase(PhasePreset preset)',
    '    {',
    '        activePhase = preset;',
    '        phaseActionDone = false;',
    '    }',
    '',
    '    public void Tick(float dt)',
    '    {',
    '        if (activePhase == null) return;',
    '        if (GMP_Player.instance != null) GMP_Player.instance.Tick(dt, AutoPlayDriver.instance != null && AutoPlayDriver.instance.isActive);',
    '        if (Input.GetMouseButtonDown(0)) TryInteractWithTarget();',
    '        TryInteractWithTarget();',
    '    }',
    '',
    '    public bool IsPhaseComplete(PhasePreset preset, float phaseTimer, float phaseRealTimer)',
    '    {',
    '        if (preset == null) return false;',
    '        return preset.gate == null || preset.gate.IsReady(Mathf.Max(phaseTimer, phaseRealTimer));',
    '    }',
    '',
    ...
    '    {',
    '        if (activePhase == null || string.IsNullOrEmpty(activePhase.targetEntity)) return;',
    '        if (EntityBindingManager.instance == null) return;',
    '        if (EntityBindingManager.instance.IsNearPlayer(activePhase.targetEntity, 2.5f)) MarkTarget(activePhase.targetEntity);',
    '    }',
    '',
    '    private void MarkTarget(string targetName)',
    '    {',
    '        if (string.IsNullOrEmpty(targetName) || EntityBindingManager.instance == null) return;',
    '        EntityBindingManager.instance.MarkEntityComplete(targetName);',
    '        phaseActionDone = true;',
    '        if (HudController.instance != null) HudController.instance.SetScore("Target: " + targetName);',
    '    }',
    '}'
  ].join('\n');
}
...
function emitPhasePresetAssets(root, phaseInfos) {
  ...
  var phaseScript = path.join(scriptsRoot, 'Common', 'PhasePreset.cs');
  ...
  for (var i = 0; i < phaseInfos.length; i++) {
    ...
    var yaml = [
      ...
      '  guideText: "' + csString(info.title) + '"',
      '  targetEntity: "' + csString(info.targetEntity || '') + '"',
      '  targetCount: ' + (gate.kind === 'entity_count' ? gate.threshold : 0),
      '  resourceKey: "' + csString(gate.kind === 'resource' ? gate.target : '') + '"',
      '  resourceThreshold: ' + (gate.kind === 'resource' ? gate.threshold : 0),
      '  damage: 0',
      '  minSeconds: 12',
      '  spawnEntities:' + (spawnLines.length ? '' : ' []')
    ].concat(spawnLines).concat([
      '  hideEntities: []',
      '  gate:',
      '    kind: "' + csString(gate.kind || '') + '"',
      '    target: "' + csString(gate.target || '') + '"',
...
function splitMainManagerForV12(root) {
  var scriptsRoot = path.join(root, 'Assets', 'Scripts');
  ...
  var phaseInfos = parsePhaseDeliveryInfo(code, phaseIds);
  ...
  writeGeneratedCs(path.join(managerDir, 'MainManager.cs'), buildThinMainManagerCode());
  writeGeneratedCs(path.join(managerDir, 'PhaseController.cs'), buildPhaseControllerCode());
  writeGeneratedCs(path.join(managerDir, 'EntityBindingManager.cs'), buildEntityBindingManagerCode(entityIds));
  writeGeneratedCs(path.join(managerDir, 'AutoPlayDriver.cs'), buildAutoPlayDriverCode());
  writeGeneratedCs(path.join(managerDir, 'HudController.cs'), buildHudControllerCode());
  writeGeneratedCs(path.join(managerDir, 'EventRuleEngine.cs'), buildEventRuleEngineCode());
  var phaseAssets = emitPhasePresetAssets(root, phaseInfos);
  return {
    changedFiles: 7 + phaseAssets,
    runtimeFiles: 0,
...
function writeHandoff(root, project, summary) {
  ...
  var lines = [
    ...
    '- 生成 PhasePreset 资产数：' + (summary.phasePresetAssets || 0),
...
function normalizeProgrammerDeliveryScripts(root) {
  ...
  var splitSummary = splitMainManagerForV12(root);
