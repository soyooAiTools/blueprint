...
function mainCode(bodyLines) {
  return [
    '// ========== 程序员交付主入口 ==========',
    '// 按反馈要求不使用 C# 拆分类；GameFlowManagerMain.cs 保持少于 1000 行。',
    '// 状态、Phase handler、preview、资源和场景辅助在普通基类文件中维护。',
    '',
    'using UnityEngine;',
    'using UnityEngine.UI;',
    'using System.Globalization;',
    '',
    'public class GameFlowManagerMain : GameFlowPhaseFlowBase',
    '{'
...
function splitMergedManagerIntoDeliveryClasses(managerDir, mergedCode) {
  ...
  var resourceLines = resourceSection;
  var previewLines = previewSection;
  ...
  var files = {
    'GameFlowStateBase.cs': classCode('GameFlowStateBase', 'MonoBehaviour', stateLines, '// 状态、实体绑定、资源和低层场景辅助。'),
    'GameFlowSceneBase.cs': classCode('GameFlowSceneBase', 'GameFlowStateBase', sceneLines, '// 对象池摆放、隐藏、缩放和生成兼容包装。'),
    'GameFlowRuntimeBase.cs': classCode('GameFlowRuntimeBase', 'GameFlowSceneBase', runtimeLines, '// 每帧运行时系统。'),
    'GameFlowInputBase.cs': classCode('GameFlowInputBase', 'GameFlowRuntimeBase', inputLines, '// 输入和玩家控制辅助。'),
    'GameFlowUiBase.cs': classCode('GameFlowUiBase', 'GameFlowInputBase', uiLines, '// UI、CTA、状态 JSON 和 phase evidence 输出。'),
    'GameFlowResourceBase.cs': classCode('GameFlowResourceBase', 'GameFlowUiBase', resourceLines, '// 资源、经济、背包和形态系统。'),
    'GameFlowPreviewBase.cs': classCode('GameFlowPreviewBase', 'GameFlowResourceBase', previewLines, '// Preview/CUA 状态导出。'),
    'GameFlowPhaseContentBase.cs': classCode('GameFlowPhaseContentBase', 'GameFlowPreviewBase', phaseContentLines, '// Phase init、点击处理和 AutoPlay 阶段副作用。'),
    'GameFlowPhaseSharedBase.cs': classCode('GameFlowPhaseSharedBase', 'GameFlowPhaseContentBase', phaseSharedLines, '// 阶段计时、进入、完成和卡点上报共享流程。'),
    'GameFlowPhaseSnapshotBase.cs': classCode('GameFlowPhaseSnapshotBase', 'GameFlowPhaseSharedBase', phaseSnapshotLines, '// 阶段入口快照辅助。'),
    'GameFlowPhaseInitBase.cs': classCode('GameFlowPhaseInitBase', 'GameFlowPhaseSnapshotBase', phaseInitLines, '// Phase 初始化处理器。'),
    'GameFlowPhaseTapBase.cs': classCode('GameFlowPhaseTapBase', 'GameFlowPhaseInitBase', phaseTapLines, '// Phase 点击处理器。'),
    'GameFlowPhaseAutoBase.cs': classCode('GameFlowPhaseAutoBase', 'GameFlowPhaseTapBase', phaseAutoLines, '// Phase AutoPlay 到达处理器。'),
    'GameFlowPhaseFlowBase.cs': classCode('GameFlowPhaseFlowBase', 'GameFlowPhaseAutoBase', phaseFlowLines, '// 阶段分发、AutoPlay tick 和 phase gate 顶层流程。'),
    'GameFlowManagerMain.cs': mainCode(mainLines)
  };
...
function rewriteReadmeForProgrammerDelivery(root) {
  ...
  var boundary = [
    ...
    '- 按反馈的五类系统管理拆分：流程控制、输入处理、资源系统、UI 和场景控制都落在普通 `GameFlow*Base.cs` 继承链中，每个脚本保持在 1000 行以内。',
  ...
  text = text.replace(
    ...
    '- `GameFlow*Base.cs` 通过普通继承链拆分输入、资源、预览、场景、UI、AutoPlay 和 Phase handler，避免单脚本超过 1000 行。\n' +
    '- `Assets/Program/Script/Manager/Entities/` 中的实体类承载领域属性，例如 `BaseBuildElement`、`BuildEntity`、`BarrackEntity`。\n'
  );
  text = text.replace(
...
function writeHandoff(root, project, summary) {
  ...
  var lines = [
    ...
    '- 按流程控制、输入处理、资源系统、UI 和场景控制五类系统管理组织维护入口。',
    '- 移除默认关闭的生成遗留路径，避免程序员维护死代码。',
    '- 生成 `Assets/Program/Script/Manager/Entities/` 领域类：`BaseBuildElement` 承载生命、攻击、等级等建筑通用属性，具体实体类继承对应基类。',
    '',
    '## 后续维护建议',
    '- 玩法主流程从 `GameFlowManagerMain.cs` 的 `Start()`、`Update()`、`CheckEventRules()` 开始阅读，再沿普通基类进入 `GameFlowPhase*Base`、`GameFlowUi/Resource/Input/Runtime/SceneBase` 和 `GameFlowStateBase`。',
