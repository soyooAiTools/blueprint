...
function optimize(htmlPath) {
  ...
  if (saved > 0) {
    ...
    return { changed: true, originalSize: originalSize, newSize: newSize, saved: saved, gzSize: gz.length };
  }

  return { changed: false, originalSize: originalSize, newSize: newSize };
}
