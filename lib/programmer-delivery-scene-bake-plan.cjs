...
function captureEntityBindings(sceneText) {
  var names = [];
  String(sceneText || '').replace(/\n\s*-?\s*mEntityName:\s*(?:"([^"]+)"|([^\n\r#]+))/g, function(_, quotedName, plainName) {
    var name = String(quotedName || plainName || '').trim();
...
function captureSceneEntityRefFields(sceneText) {
  var names = [];
  String(sceneText || '').replace(/\n\s*(m[A-Z][A-Za-z0-9_]*):\n\s*mSceneObject:\s*\{fileID:\s*[^}]+\}/g, function(_, fieldName) {
    fieldName = String(fieldName || '').trim();
    if (names.indexOf(fieldName) < 0) names.push(fieldName);
    return _;
  });
  return names.sort();
}

function captureSceneEntityRefIds(root) {
  var file = path.join(root, 'Assets', 'Scripts', 'Game', 'Level', 'GMP_SceneEntityRefs.cs');
  if (!fs.existsSync(file)) return [];

  var names = [];
  fs.readFileSync(file, 'utf8').replace(/AddSceneEntity\(\s*"([^"]+)"/g, function(_, name) {
    name = String(name || '').trim();
    if (names.indexOf(name) < 0) names.push(name);
    return _;
  });
...
function buildBakeActions(summary, sceneStats, scripts) {
  ...
  actions.push({
    ...
    output: 'serialized manager refs, HUD refs, camera refs and scene entity refs',
...
function buildSceneBakePlan(root, options) {
  ...
  var entityBindings = captureEntityBindings(sceneText);
  var sceneEntityRefFields = captureSceneEntityRefFields(sceneText);
  var sceneEntityRefIds = captureSceneEntityRefIds(root);
  var sceneStats = {
    exists: !!sceneText,
    ...
    entityBindingCount: entityBindings.length,
    entityBindingNames: entityBindings,
    sceneEntityRefFieldCount: sceneEntityRefFields.length,
    sceneEntityRefFields: sceneEntityRefFields,
    sceneEntityRefCount: sceneEntityRefIds.length,
    sceneEntityRefIds: sceneEntityRefIds,
  ...
  return {
    ...
    strategy: {
      ...
      keepSceneEntityRefsAsData: true,
      keepEntityBindingTableAsData: entityBindings.length > 0,
