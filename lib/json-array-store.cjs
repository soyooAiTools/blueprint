 ...
 *
 *  - writeJSONAtomic(file, value): identical to writeArrayAtomic but
 *    accepts any JSON-serializable value. Use this for object-shaped
 *    state files (e.g. auto-fix-state.json with `{ pendingCommits: ... }`).
 *
 * All helpers are pure-ish (no module-level state) and synchronous, so
...
module.exports = {
  ...
  // Backward-compat alias: PR-11 callers still use writeArrayAtomic.
  writeArrayAtomic: writeJSONAtomic,
};
