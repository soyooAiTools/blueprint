 ...
 *
 * Supports: Gemini, Claude (Anthropic), Doubao (豆包)
...
function GeminiProvider(config) {
  ModelProvider.call(this, 'gemini', config);
  // config: { apiKey, baseUrl, model }
  this.apiKey = config.apiKey || process.env.GEMINI_API_KEY;
  this.baseUrl = config.baseUrl || process.env.GOOGLE_GEMINI_BASE_URL;
  this.model = config.model || process.env.GEMINI_MODEL || 'gemini-3.1-pro-preview';
}
GeminiProvider.prototype = Object.create(ModelProvider.prototype);

GeminiProvider.prototype.generate = function(prompt, options) {
  options = options || {};
  var self = this;
  var GoogleGenAI;
  try {
    ...
    return Promise.reject(new Error('Missing @google/genai package'));
  }

  var ai = new GoogleGenAI({ apiKey: self.apiKey, httpOptions: { baseUrl: self.baseUrl } });
  var contents = typeof prompt === 'string' ? prompt : (prompt.user || prompt.content || JSON.stringify(prompt));
  ...
  return ai.models.generateContent({
    ...
    return { text: result.text || '', usage: result.usage, model: self.model, provider: 'gemini' };
  });
};
...
function DoubaoProvider(config) {
  ...
  // config: { apiKey, model }
  this.apiKey = config.apiKey || process.env.DOUBAO_API_KEY || process.env.GEMINI_API_KEY;
 ...
 * @returns {Promise<{doubao: object, gemini: object, claude: object}>}
 */
function healthCheck() {
  ...
  // Doubao
  if (process.env.DOUBAO_API_KEY || process.env.GEMINI_API_KEY) {
  ...
  if (process.env.GEMINI_API_KEY) {
    ...
    var t2 = Date.now();
    checks.push(
      gemini.generate("ping", { maxTokens: 5, timeoutMs: 10000 })
        .then(function() { results.gemini = { status: "ok", latencyMs: Date.now() - t2 }; })
        .catch(function(e) { results.gemini = { status: "error", latencyMs: Date.now() - t2, error: e.message }; })
    );
  }
