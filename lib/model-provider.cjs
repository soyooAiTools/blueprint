...
function ClaudeProvider(config) {
  ...
  //    (same relay Claude Code CLI uses, reliably up)
...
ClaudeProvider.prototype.generate = function(prompt, options) {
  ...
  if (self.useAnthropicNative) {
    // Anthropic-native /v1/messages on crs.mindrix.app — the same relay Claude Code CLI uses.
...
function healthCheck() {
  ...
  // Claude — Anthropic-native /v1/messages via crs.mindrix.app (preferred) or
  // OpenAI-compat /chat/completions via sub.mindrix.app (legacy fallback).
  // ClaudeProvider auto-selects based on ANTHROPIC_AUTH_TOKEN env var.
  if (process.env.ANTHROPIC_AUTH_TOKEN || process.env.OPENAI_API_KEY) {
