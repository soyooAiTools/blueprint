 ...
 *
 * Supports: Doubao (豆包), optional legacy Claude provider (disabled by default)
...
function isClaudeDisabled() {
  return envFlag('BLUEPRINT_DISABLE_CLAUDE') || !envFlag('BLUEPRINT_ENABLE_CLAUDE');
}
...
ClaudeProvider.prototype.generate = function(prompt, options) {
  if (isClaudeDisabled()) {
    return Promise.reject(new Error('Claude provider disabled; set BLUEPRINT_ENABLE_CLAUDE=1 only for legacy use'));
  }
  options = options || {};
  var self = this;
...
ClaudeProvider.prototype.generateVision = function(imageBase64, textPrompt, options) {
  if (isClaudeDisabled()) {
    return Promise.reject(new Error('Claude vision provider disabled; set BLUEPRINT_ENABLE_CLAUDE=1 only for legacy use'));
  }
  options = options || {};
...
DoubaoProvider.prototype.generateVision = function(imageBase64, textPrompt, options) {
  options = options || {};
  var images = Array.isArray(imageBase64) ? imageBase64 : [imageBase64];
  var apiKey = options.apiKey || this.apiKey || process.env.DOUBAO_API_KEY;
  var baseUrl = options.baseUrl || process.env.DOUBAO_API_BASE || 'https://ark.cn-beijing.volces.com/api/v3';
  var model = options.model || process.env.DOUBAO_VISION_MODEL || process.env.VLM_MODEL || 'doubao-1-5-vision-pro-32k-250115';
  var maxTokens = options.maxTokens || options.maxOutputTokens || 512;
  var timeoutMs = options.timeoutMs || 60000;
  if (!apiKey) return Promise.reject(new Error('MODEL_FATAL: Doubao vision API key missing'));
  ...
  var endpointUrl = baseUrl.replace(/\/$/, '') + '/chat/completions';
    ...
    var req = mod.request({
      ...
      res.on('end', function() {
        ...
        try {
          var resp = JSON.parse(raw);
          if (resp.error) {
            var errMsg = resp.error.message || JSON.stringify(resp.error);
            var isModelFatal = /quota|insufficient|\b401\b|\b402\b|\b403\b|invalid.?api.?key|unauthoriz|authentication.?fail|access.?denied|billing/i.test(errMsg);
            return reject(new Error((isModelFatal ? 'MODEL_FATAL: ' : '') + 'Doubao vision API: ' + errMsg));
          }
          if (res.statusCode && (res.statusCode === 401 || res.statusCode === 402 || res.statusCode === 403)) {
            return reject(new Error('MODEL_FATAL: Doubao vision API: HTTP ' + res.statusCode + ' ' + raw.slice(0, 200)));
          }
          if (res.statusCode && res.statusCode >= 400) {
            return reject(new Error('Doubao vision API: HTTP ' + res.statusCode + ' ' + raw.slice(0, 200)));
          }
          var text = (resp.choices && resp.choices[0] && resp.choices[0].message && resp.choices[0].message.content) || '';
          resolve({ text: text, provider: 'doubao-vision', model: model, usage: resp.usage || null });
        } catch(e) {
          reject(new Error('Doubao vision parse error: ' + e.message));
        }
      });
    });
    req.on('timeout', function() {
      req.destroy(new Error('Doubao vision timeout after ' + timeoutMs + 'ms'));
    });
    req.on('error', reject);
    req.write(apiBody);
    req.end();
...
function createProvider(type, config) {
  switch (type) {
    case 'claude':
      if (isClaudeDisabled()) throw new Error('Claude provider disabled; set BLUEPRINT_ENABLE_CLAUDE=1 only for legacy use');
...
function createDefaultChain() {
  ...
  // Optional legacy fallback: disabled by default and hard-blocked by BLUEPRINT_DISABLE_CLAUDE.
  if (!isClaudeDisabled() && process.env.BLUEPRINT_ENABLE_CLAUDE_FALLBACK === '1' && process.env.OPENAI_API_KEY) {
    providers.push({
...
/**
 * Health check — ping configured providers
 * Probed providers:
 *   - doubao  : 豆包 Seed (text + CUA VLM 也是豆包 vision)
 *   - claude  : disabled unless BLUEPRINT_ENABLE_CLAUDE=1
...
function healthCheck() {
  ...
  if (isClaudeDisabled()) {
    results.claude = { status: 'disabled', mode: 'codex-only-default' };
  } else {
...
module.exports = {
  ...
  isClaudeDisabled: isClaudeDisabled,
};
