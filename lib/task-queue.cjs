...
class TaskQueue {
  ...
  reclaimStale(orphanThresholdSec, workerDeadSec) {
    ...
    for (var i = 0; i < staleTasks.length; i++) {
      ...
      ).run('[watchdog] Re-queued: ' + reason, task.id);

      this._recordHistory(task.id, task.status, 'pending', 'watchdog', reason);
      fixes.push('[fix] ' + task.id + ' -> pending (' + reason + ')');
    }
  ...
  stats() {
    ...
    for (var j = 0; j < workers.length; j++) {
      if (workers[j].last_seen && (now - new Date(workers[j].last_seen).getTime()) < 90000) online++;
    }
