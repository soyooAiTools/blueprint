...
// 2026-04-21: outer-retry fingerprint de-dup. See _checkOuterRetryFingerprint below.
var _normalizeFingerprint;
try {
  _normalizeFingerprint = require('../engine/metrics.cjs').normalizeFingerprint;
} catch (e) {
  _normalizeFingerprint = function(s) { return String(s || '').slice(0, 120); };
}

const DEFAULT_DB_PATH = path.join(__dirname, '..', 'server-data', 'blueprint.db');
const MAX_INFRA_RETRIES = 5;
const MAX_CODE_RETRIES = 5;
const OUTER_RETRY_FP_FATAL_AT = 2;
...
class TaskQueue {
  ...
  _prepareStatements() {
    this._stmts = {
      ...
      touchTaskFromHeartbeat: this.db.prepare(
        "UPDATE tasks SET updated_at = datetime('now') " +
        "WHERE id = ? AND assigned_to = ? AND status IN ('assigned', 'processing', 'building', 'preview_ready')"
      ),
      ...
      findStaleTasks: this.db.prepare(
        ...
        "WHERE t.status IN ('assigned', 'processing', 'building', 'preview_ready') " +
  ...
  report(taskId, status, data) {
    ...
    var outerMetaUpdate = null; // set by outer-retry fingerprint check below

    if (status === 'done' || status === 'cua_passed') {
      newStatus = status;
      assignedTo = null;
    } else if (oldStatus === 'preview_ready' && (status === 'processing' || status === 'building' || status === 'developing')) {
      // `preview_ready` is an intermediate UI milestone: visual-check has
      // written a playable preview, but runtime-contract / CUA may still be
      // running. The queue must move back to an active status so heartbeat and
      // stale recovery can reclaim the task if the worker dies mid-validation.
      newStatus = status;
      retryAfter = null;
      ...
      if (isInfra) {
        ...
        codeRetryCount++;
        // 2026-04-21: outer-retry fingerprint de-dup. Monitoring report 04-21
        // showed top-1/top-2 failures (25/61) are nested "same CODE error
        // repeated 3 rounds, fix-loop not converging" — the inner CUA fingerprint
        // circuit breaker trips after 3 rounds, outer retry re-enqueues, inner
        // burns 3 more rounds on the SAME root cause. Persist normalized
        // fingerprint across outer retries; if it repeats N times → skip
        // remaining retries, go straight to permanently_failed.
        outerMetaUpdate = this._parseJson(task.metadata_json, {});
        outerMetaUpdate.outerFpHistory = Array.isArray(outerMetaUpdate.outerFpHistory) ? outerMetaUpdate.outerFpHistory : [];
        var _currentFp = _normalizeFingerprint(message || '');
        outerMetaUpdate.outerFpHistory.push(_currentFp);
        if (outerMetaUpdate.outerFpHistory.length > 10) outerMetaUpdate.outerFpHistory.splice(0, outerMetaUpdate.outerFpHistory.length - 10);
        var _repeatCount = 0;
        for (var _hi = outerMetaUpdate.outerFpHistory.length - 1; _hi >= 0; _hi--) {
          if (outerMetaUpdate.outerFpHistory[_hi] === _currentFp) _repeatCount++;
          else break;
        }
        var _fpFatal = (_currentFp && _currentFp !== 'unknown' && _repeatCount >= OUTER_RETRY_FP_FATAL_AT);
        if (_fpFatal) {
          newStatus = 'failed';
          statusMessage = 'Outer-retry fingerprint FATAL: "' + String(_currentFp).substring(0, 100) + '" repeated ' + _repeatCount + 'x across outer retries — saved ' + Math.max(0, MAX_CODE_RETRIES - codeRetryCount) + ' burn rounds';
        } else if (codeRetryCount <= MAX_CODE_RETRIES) {
    ...
    if (newStatus === 'failed' || newStatus === 'done' || newStatus === 'cua_passed' || newStatus === 'completed' || newStatus === 'cancelled') {
      assignedTo = null;
    }
    ...
    if (data.qualityData || outerMetaUpdate) {
      var metadata = outerMetaUpdate || this._parseJson(metadataJson, {});
      if (data.qualityData) {
  ...
  cancel(taskId, actor) {
    ...
    if (!task) return null;
    this.db.prepare(
      "UPDATE tasks SET status = 'cancelled', assigned_to = NULL, assigned_at = NULL, retry_after = NULL, " +
      "status_message = ?, updated_at = datetime('now') WHERE id = ?"
    ).run('cancelled', taskId);
  ...
  heartbeat(workerId, status, currentTask, uptime, ip, port) {
    ...
    this._stmts.upsertHeartbeat.run(
      workerId,
      status || 'unknown',
      typeof currentTask === 'object' ? JSON.stringify(currentTask) : currentTaskId,
      uptime || 0,
      ip || null,
      port || null
    );
    // Long review / codegen rounds can legitimately run for many minutes without
    // emitting a task status update. If the heartbeat still points at the active
    // task, treat it as forward progress so watchdog stale-task recovery does not
    // reclaim healthy in-flight work.
    if (currentTaskId) {
      this._stmts.touchTaskFromHeartbeat.run(currentTaskId, workerId);
    }
  }
    ...
    for (var i = 0; i < staleTasks.length; i++) {
      ...
      if (!task.assigned_to) {
        ...
        if (worker && (!worker.current_task || worker.current_task === 'null')) {
          ...
          // and may still resume checkpointed tasks while heartbeat reports idle.
          // Past that grace period, an idle heartbeat with no current_task means
          // the queue and worker have diverged and the task will never make
          // forward progress unless we re-queue it.
          if ((worker.uptime || 0) < 180) {
            continue;
          }
          reason = task.status === 'assigned'
            ? 'worker idle desync'
            : 'worker idle desync after restart';
        } else {
          continue;
        }
      }
  ...
  resetForRerun(taskId, message, actor) {
    var task = this.get(taskId);
    if (!task) return null;
    var nextMessage = message || '[rerun] reset to pending';
    this.db.prepare(
      "UPDATE tasks SET status='pending', assigned_to=NULL, assigned_at=NULL, retry_after=NULL, " +
      "status_message=?, fail_count=0, infra_retry_count=0, code_retry_count=0, metadata_json=?, updated_at=datetime('now') WHERE id=?"
    ).run(
      nextMessage,
      scrubTaskMetadataJsonForRerun(task.metadata_json),
      taskId
    );
    this._recordHistory(taskId, task.status, 'pending', actor || 'server', nextMessage);
    var timeline = this._parseJson(task.timeline_json, []);
    timeline.push({ status: 'pending', message: nextMessage, workerId: actor || 'server', timestamp: Date.now() });
    if (timeline.length > 50) timeline.splice(0, timeline.length - 50);
    this.db.prepare('UPDATE tasks SET timeline_json = ? WHERE id = ?').run(JSON.stringify(timeline), taskId);
    return this.get(taskId);
  }
