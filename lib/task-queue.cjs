    ...
    for (var i = 0; i < staleTasks.length; i++) {
      ...
      if (!task.assigned_to) {
        ...
        if (worker && (!worker.current_task || worker.current_task === 'null')) {
          ...
          // A live worker with no current_task is only actionable for tasks that
          // never really started (`assigned`). For tasks already in
          // `processing/building`, this usually means a restart or heartbeat/task
          // mismatch rather than a true code failure. Let explicit worker-dead
          // detection or night-monitor stuck recovery handle those cases instead
          // of burning code retries and permanently failing healthy work.
