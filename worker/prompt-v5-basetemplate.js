...
function parseBlueprintToPromptV5(blueprint, opts) {
  ...
  lines.push('');
  lines.push('## 重要规则:');
  lines.push('- ✅ 骨架 AutoPlayUpdate() 已处理玩家基础移动；OnAutoPlayArrive 由它在玩家到达每个 target 时调用');
  lines.push('- ✅ 每个 phase 的关键实体都必须在 OnAutoPlayArrive 对应 case 里被移动/显示/隐藏 (> 1.5 单位)');
  lines.push('- ✅ NPC 移动用 Vector3.MoveTowards(pos, target, speed * dt)，不要瞬移');
  lines.push('- ✅ 每个 Phase 要有明显的视觉变化（对象出现/移动/颜色变化/UI 更新）');
  lines.push('- ⛔ OnAutoPlayArrive 里禁止直接赋值 xxxState=N / xxxDone=true / PlayerActed=true');
