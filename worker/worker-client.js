...
async function processTask(task) {
  ...
  try {
      ...
      for (let cuaRound = 1; cuaRound <= MAX_CUA_ROUNDS; cuaRound++) {
        try {
          ...
          if (cuaResult.report && cuaResult.report.diagnostics) {
            ...
            if (!diag.engineReady) {
              cuaFeedbackText += '\n\nâš ï¸ ENGINE NOT INITIALIZED - This is likely an infrastructure issue, not a code issue.';
    ...
    if (!htmlOutputExists) {
      ...
      for (let previewRound = 1; previewRound <= MAX_PREVIEW_FIX_ROUNDS; previewRound++) {
        ...
        const previewFeedback = {
          ...
          instruction: `Preview check failed after build. Issue: ${previewResult.error}ã€‚` +
            (previewResult.consoleErrors?.length ? `Browser console errors: ${previewResult.consoleErrors.slice(0, 5).join('; ')}ã€‚` : '') +
    ...
    for (let cuaRound = 1; cuaRound <= MAX_CUA_ROUNDS; cuaRound++) {
      try {
        ...
        if (cuaResult.report) {
          ...
          if (allUncovered) {
            visualContext += '\n\nâš ï¸ Critical: All shots uncovered' + (cuaResult.report.scriptCoverage || []).length + ')ã€‚This usually means:\n'
    ...
    notifyEvent(taskId, 'done', `ðŸŽ‰ Task done! Build ${buildResult.buildTime}s, CUA passed, waiting for review`, { projectName: task.projectName });
    ...
    const runWithRetry = async () => {
      ...
      try {
        ...
        if (retries < MAX_TASK_RETRIES && !e.noRetry) {
          ...
          log(`ðŸ”„ Task retry ${retries + 1}/${MAX_TASK_RETRIES} in ${delay/1000}s: ${e.message.slice(0, 100)}`, task.taskId);
