...
14. 代码要让人类程序员能直接接手：变量名要说明业务含义；只保留会被调用的方法；只有一个调用点且只包一两行的逻辑直接内联；不要为了“看起来分层”拆一堆函数和变量。
15. 场景问题优先由 AIBridge/MCP/Editor 处理：Missing Mono Script、Rigidbody、Collider、Animator 等配置不要在业务代码里反复 Find/AddComponent/修复。
16. 单例/管理器用场景预挂实例和 serialized refs；单例类里不要再塞静态 Init/Get/Return 这类工作流方法。
17. 性能和兜底：距离门槛用 `sqrMagnitude`；必要兜底才写，不能为理论上进不去的分支堆查找、创建、修复逻辑。
