// Capture full error stacks from page load
...
(async () => {
  ...
  page.on('pageerror', err => {
    errors.push({ msg: err.message.substring(0, 300), stack: err.stack ? err.stack.substring(0, 500) : 'no stack' });
  });
  ...
  console.log('\n=== ERROR STACKS ===');
  for (let i = 0; i < Math.min(errors.length, 8); i++) {
    console.log(`\nError ${i+1}: ${errors[i].msg}`);
    console.log(`Stack: ${errors[i].stack}`);
  }
