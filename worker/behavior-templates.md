...
> 本文件只给 Luna/WebGL staging 代码参考。程序员 Unity 交付版必须由 AIBridge/MCP 做 Inspector/scene hydration，引用进入 `GMP_EntityBindingManager.mBindings` 或 `[SerializeField]` 字段；不要把这里的平行数组、运行时 `Find`、`GameSceneCtrl` / `SceneObjectRegistry` 隐藏运行时对象表或一次性模板拆法照搬成最终交付结构。
> 程序员可交付反馈规则：一节点一主脚本；无生命周期能力默认用普通 C# 类；MoveSpeed 归 MovementComponent/移动能力、交互半径归 Trigger/Interaction、背包容量归 Inventory；变量名要说明业务含义；只保留会被调用的方法；Init 未被调用就删除，依赖 Awake/Start 时不保留并行 Init；固定 Player 引用缺失只 `Debug.LogError`，不写查找/创建/修组件的 fallback；phase 是连续试玩流程，不重置全场；AIBridge 预水合后删除 primitive builder、source spec helper 等临时脚本；场景遗留、Missing Mono Script 和组件配置优先由 Editor/MCP 修掉。
