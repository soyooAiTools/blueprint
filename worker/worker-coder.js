...
const UNITY_PROGRAM_ARCHITECTURE_RULES_TEXT = [
  ...
  '- 游戏入口必须检测 `Screen.width/Screen.height` 变化并发布 `GMP_LevelEventNames.ScreenChanged` + `GMP_ScreenChangeEvent`；监听方必须显式 Subscribe，并在 OnDestroy 用 UnSubScribe 注销。',
  ...
  '- `GMP_TipsManager.mTipText` 必须绑定场景预设 Text（如 Text_StepToast），TipsManager 不按名字扫描 Text，也不在代码里硬改 RectTransform 布局/字号/样式；`GMP_CameraController` 交付默认正交相机，phase/end 构图只写目标状态并由 LateUpdate 平滑收敛。',
...
var GENERATE_PROMPT = [
  ...
  '  Canvas canvas = uiCanvas;                         // pre-created Canvas; do not create a delivery Canvas at runtime',
  ...
  '- Direct Text.font / Text.fontSize / Text.alignment writes — Luna UI.Text backing element may be null; use GFM_UI and only update .text afterward',
  ...
  '- AIBridge evidence must come from actually running AIBridgeCLI: resolve `AIBRIDGE_CLI` or `command -v AIBridgeCLI`, run `AIBridgeCLI harness status` and `AIBridgeCLI editor get_state --timeout <ms>`, then record stdout/stderr/exit code in MCP_HYDRATION_REPORT.json, AIBRIDGE_ATTEMPT_REPORT.json, or AIBRIDGE_REAL_RUN_REPORT.json. If the CLI exists but Unity Editor/AIBridge times out, report editor-timeout, not CLI not found.',
  '- Editor hydration 未完成时不能把 Unity 包标记为最终交付认证通过；static YAML / file-level checks are file audit only and cannot replace opening the project in Unity Editor, resolving Inspector refs, and completing AIBridge editor get_state / scene hydration.',
  ...
  '  - BasicExtensions, ReturnPool, EventManager (do not inherit MonoSingleton<T>; final managers use scene-mounted mInstance/instance)',
  ...
  '- You CAN use: PoolManager, AudioManager, SimpleAudioManager, CTAManager, LunaManager, GameConstants, GameData, DOTween',
  ...
  '  Canvas canvas = uiCanvas;                            // pre-created by skeleton/scene',
  '  if (canvas == null) return;                          // do not create a delivery Canvas at runtime',
  '  GFM_UI.CreateText(canvas, "Hello", Vector2.zero, 28); // staging only; final UI nodes are scene refs',
...
var FIX_PROMPT = [
  ...
  '```',
  'Canvas canvas = uiCanvas;                                                        // pre-created Canvas; do not create delivery Canvas at runtime',
  ...
  '- Use the pre-created `uiCanvas`; do not call GFM_UI.CreateCanvas() in delivery-oriented logic.',
  ...
  '- For programmer delivery, actually run AIBridgeCLI before claiming hydration status: resolve `AIBRIDGE_CLI` or `command -v AIBridgeCLI`, run `AIBridgeCLI harness status` and `AIBridgeCLI editor get_state --timeout <ms>`, and record stdout/stderr/exit code. CLI found + Editor timeout must be reported as editor-timeout, not CLI not found.',
  '- Editor hydration 未完成时不能把 Unity 包标记为最终交付认证通过；static YAML / file-level checks are file audit only and cannot replace live Unity Editor hydration.',
  ...
  '- Do NOT directly assign Text.font/Text.fontSize/Text.alignment/Text.horizontalOverflow in generated code; create text through GFM_UI and only set .text for updates.',
